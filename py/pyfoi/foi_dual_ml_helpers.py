...
from pyfoi import foi_dml_a1_non_xolam_helpers as avnx
...
def _alef_vav_xolam_pattern(prefix="", suffix=""):
    return prefix + _ALEF_VAV_XOLAM_PATTERN + suffix


def _vav_alef_xolam_pattern(prefix="", suffix=""):
    return prefix + _VAV_ALEF_XOLAM_PATTERN + suffix


def _alef_vav_xolam_pattrec(end_of_path, prefix="", suffix=""):
    top_path_part_av = "alef-dual-ml-alef-first"
    return (
        _alef_vav_xolam_pattern(prefix=prefix, suffix=suffix),
        (top_path_part_av, end_of_path),
    )


def _vav_alef_xolam_pattrec(end_of_path, prefix="", suffix=""):
    top_path_part_va = "alef-dual-ml-alef-second"
    return (
        _vav_alef_xolam_pattern(prefix=prefix, suffix=suffix),
...
_ALEF_VAV_XOLAM_PATTERN = hpo.XOLAM + _NON_LETT_STAR + hl.ALEF + hl.VAV
_VAV_ALEF_XOLAM_PATTERN = hl.VAV + hpo.XOLAM + hl.ALEF
_PATTRECS_AV = (  # Order matters! First one to match "wins"!
    _alef_vav_xolam_pattrec("c-smol", prefix=_PRE_AV_SIN_MEM, suffix=hl.LAMED),
    _alef_vav_xolam_pattrec("c-xatot", prefix=_PRE_AV_XET_TET, suffix=hl.TAV),
    _alef_vav_xolam_pattrec("c-m.lot", prefix=_PRE_AV_MEM_LAM, suffix=hl.TAV),
    _alef_vav_xolam_pattrec("b-ot", suffix=hl.TAV),
    _alef_vav_xolam_pattrec("a-misc", suffix=_NOT_DOM_OR_XOLAM),
)
_PATTRECS_VA = (  # Order matters! First one to match "wins"!
    # None 1st arg means don't record this
    _vav_alef_xolam_pattrec(None, suffix=_ACCAM_STAR_THEN_VAV_MALE_VOWEL),
    _vav_alef_xolam_pattrec(None, suffix=_VOWEL_MARK),
    _vav_alef_xolam_pattrec("b-not-final", suffix=rh.LETT),
    _vav_alef_xolam_pattrec("a-misc"),
)
