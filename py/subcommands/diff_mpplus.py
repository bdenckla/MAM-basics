...
def generated_artifact_names() -> tuple[str, ...]:
    """Return every artifact regenerated by ``run_all``, in output order."""
...
def run_all(change_log_dir=CHANGE_LOG_DIR):
    ...
    unpinned_info = run_unpinned_latest(change_log_dir)
...
def run_unpinned_latest(change_log_dir=CHANGE_LOG_DIR):
    ...
    if count == 0:
        print(
            ...
            f"{unpinned_latest_html}, {unpinned_latest_json}"
        )
        return None
...
def _generated_artifact_problems(
    ...
    """Return missing or byte-different generated artifact problems."""
    problems = []
    generated_dir = Path(generated_dir)
    tracked_dir = Path(tracked_dir)
    for name in generated_artifact_names():
        generated_path = generated_dir / name
        tracked_path = tracked_dir / name
        if not tracked_path.is_file():
            problems.append(f"Missing tracked artifact: {name}")
        elif not generated_path.is_file():
            problems.append(f"Missing regenerated artifact: {name}")
        elif tracked_path.read_bytes() != generated_path.read_bytes():
            problems.append(f"Differing artifact: {name}")
    return problems


def check_all():
    """Regenerate outside the repository and fail if tracked artifacts are stale."""
    with TemporaryDirectory(prefix="mam-change-log-check-") as generated_dir:
        run_all(generated_dir)
        problems = _generated_artifact_problems(generated_dir)
    if problems:
        for problem in problems:
            print(problem)
        raise SystemExit(1)
    print("MAM change-log artifacts are current.")
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--check",
        action="store_true",
        help="Regenerate all artifacts in a temporary directory and check tracked copies",
    )
    parser.add_argument(
        "--all",
        action="store_true",
