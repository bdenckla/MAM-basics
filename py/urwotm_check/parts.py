...
``py/author_site/site_data.py`` (doc/PLAN-unify-the-document-index.md). Since
2026-09-18 the named four-part series is nested under "Pointing and
cantillation" on that topical index. Nothing fetches these ids any more --
``src/`` holds the frozen text -- but they are the only record of where that
text came from, so a later reader can reach the document a page was ported
from.
"""
