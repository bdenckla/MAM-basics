...
This file's canonical source is in MAM-basics (py/main_tmpl_survey_toy.py).
It is copied to MAM-parsed/py-examples/ as part of the parse-go pipeline.

When run from the MAM-parsed repo root (e.g. as a standalone example):

    python py-examples/main_tmpl_survey_toy.py
