...
def _match_with_tokens(expected, actual) -> bool:
    ...
    if isinstance(expected, list):
        if not isinstance(actual, list):
            return False
        return _match_list_with_tokens(expected, actual)
...
def _match_any_string_tokens(pattern, actual) -> bool:
    ...
    assert False, f"invalid any-string token payload: {pattern!r}"


def _match_string_parts(parts: list, actual: str) -> bool:
    """Match string parts where _MID_STRING_GAP_TOKEN means any segment."""
    for part in parts:
...
def _match_list_with_tokens(expected: list, actual: list) -> bool:
    """List matcher where _LIST_GAP_TOKEN means a wildcard subsequence."""
    ...
    def _rec(i: int, j: int) -> bool:
        ...
        if _match_with_tokens(token, actual[j]):
            result = _rec(i + 1, j + 1)
            memo[key] = result
            return result
