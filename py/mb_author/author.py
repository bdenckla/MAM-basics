...
from mb_author import dollar_sub_g
...
_ROMANIZED = {
    ...
    "$germuq": "geresh muqdam",
    "$meteg": "meteg",
    # The two readings of U+05BD together, for prose that means the mark
    # rather than either reading of it. Silluq has no key of its own: it is
    # meteg's verse-final reading, never a mark you can name from a codepoint.
    "$meteg_sl_silluq": "meteg/silluq",
    "$rafeh": "rafeh",
    "$xumash": "ḥumash",
    "$masorah": "masorah",
    ...
    "$xet": "ḥet",
    "$xolam": "ḥolam",
    "$xolam_xaser_xx": "ḥ. ḥ.",  # a bit like germuq_gm
    "$malei": "malei",
    "$xiriq": "ḥiriq",
    "$xiriq_qatan": "ḥiriq qatan",
    "$kaf": "kaf",
    "$kaf_rafeh": "kaf rafeh",
    "$khaf": "khaf",
    ...
    "$shuruq": "shuruq",
    "$xataf": "ḥataf",
    "$xataf_shewa": "ḥataf shewa",
    "$xataf_patax": "ḥataf pataḥ",
    "$xataf_segol": "ḥataf segol",
    "$nun": "nun",
    "$pashta": "pashta",
    "$xolam_malei": "ḥolam malei",
    "$iluy": "illuy",
}
...
_DOLLAR_SUB_DISPATCH = {
    ...
    **_rom_with_cap("$xolam_xaser", "ḥolam ḥaser"),
    **_rom_with_cap("$yored", "yored"),
    **_rom_with_cap("$yetiv", "yetiv"),
    **_rom_with_cap("$darga", "darga"),
    **_rom_with_cap("$mahapakh", "mahapakh"),
    **_rom_with_cap("$munax", "munaḥ"),
    **_rom_with_cap("$munleg", "munaḥ legarmeh"),
    **_rom_with_cap("$pazer", "pazer"),
    **_rom_with_cap("$legarmeh", "legarmeh"),
    # Moved here from _ROMANIZED for the cap key: the ArtScroll review opens a
    # bullet with "Maqaf appears in neither ...".
    **_rom_with_cap("$maqaf", "maqaf"),
    **_rom_with_cap("$paseq", "paseq"),
    **_rom_with_cap("$tsinnor", "tsinnor"),
    **_rom_with_cap("$qamats", "qamats"),
    **_rom_with_cap("$dexi", "deḥi"),
    #
    **_ANCHORS,
    **_UNICODE_NAME_SC,
    **_ABBR_SC,
