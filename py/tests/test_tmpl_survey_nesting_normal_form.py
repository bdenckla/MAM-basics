...
def _read_top_stack(summary_entry):
    stack = summary_entry.get("stack")
    if stack is not None:
        return stack
    return summary_entry["stack_path"]
...
class TestTmplSurveyNestingNormalForm(unittest.TestCase):
    ...
    def test_top_path_reader_supports_stack_path_alias(self):
        self.assertEqual(
            "E/A/B/C",
            _read_top_stack({"stack_path": "E/A/B/C", "count": 2}),
        )
    ...
    def test_assert_stack_counts_in_normal_form_accepts_example_path_alias(self):
        legacy_violations = [
            {
                "caller": "מ:דחי",
                "callee": "מ:קמץ",
                ...
                "relation": "descending",
                "count": 1,
                "example_path": "E/מ:דחי/מ:קמץ",
            }
        ]

        with mock.patch.object(
            nnf,
            "find_rank_violations",
            return_value=legacy_violations,
        ):
            with self.assertRaises(AssertionError) as ctx:
                nnf.assert_stack_counts_in_normal_form({}, dataset_name="unit")

        self.assertIn("example=E/מ:דחי/מ:קמץ", str(ctx.exception))

    def test_assert_stack_counts_follow_expanded_grammar_accepts_example_path_alias(self):
        legacy_violations = [
            {
        ...
        with mock.patch.object(
            nnf,
            "find_expanded_grammar_violations",
            return_value=legacy_violations,
        ):
            with self.assertRaises(AssertionError) as ctx:
                nnf.assert_stack_counts_follow_expanded_grammar(
                    {},
                    {"allowed_edges": [], "must_precede": []},
                    dataset_name="unit",
                )

        self.assertIn("example=R/C", str(ctx.exception))
