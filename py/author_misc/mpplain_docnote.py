...
"""
HTML document: Documentation template (docnote / נוסח) in plain format.

To regenerate the output HTML, run from the repo root::

    .venv/Scripts/python.exe py/main_authored.py gen-mam-parsed-docs

Output goes to gh-pages/MAM-parsed/plain/html/mpplain_docnote.html.
"""
...
def gen_html_file(tdm_ch, claims: ClaimCollection):
    """Generate mpplain_docnote.html in the given output directory."""
...
def build_body(*, claims: ClaimCollection):
    json_docnote_plain = cmn.emit_claim_by_id(
        claims=claims,
        claim_id="mp.plain.example.docnote",
    )
    ...
    return [
        ...
        json_block.json_block_raw_html(json_docnote_plain),
    ]
