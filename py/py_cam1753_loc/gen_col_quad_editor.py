...
COORD_DIR = cam1753_paths.col_quads_dir()
...
def main():
    """Write the column-quadrilateral editor for one page into ``.novc/``.

    The argument parsing below sat directly under ``if __name__ == "__main__"`` in
    codex-index-cam1753 until Phase 3 of
    ``doc/PLAN-evacuate-python-from-codex-index-trio.md``, 2026-08-22, which is why
    it is a function now: ``main_cam1753_gen_col_quad_editor.py`` has to be able to
    import it.  ``py_ac_loc.gen_col_quad_editor`` was given a ``main()`` for the same
    reason by Phase 1 of the same plan.
    """
    if len(sys.argv) < 2:
        print(
            "Usage: .venv/Scripts/python.exe py/main_cam1753_gen_col_quad_editor.py"
            " <page_id>"
        )
        print(
            "  e.g.: .venv/Scripts/python.exe py/main_cam1753_gen_col_quad_editor.py"
            " 0073B"
        )
        sys.exit(1)
