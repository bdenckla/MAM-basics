...
"""Verify that the aleppo word-finding code finds every quirkrec word,
and that the found location matches the expected qr-ac-loc.
...
def main():
    ...
    for rec in data:
        ...
        try:
            ...
            try:
                col, line_num, word_idx, line_words, match_method = (
                    find_word_in_linebreaks(LB_DIR, page_ids, "Job", ch, v, lb_word)
                )
        ...
        # The finder returns 0-based word_idx; qr-ac-loc uses 1-based
        ...
        if found_word_1based != expected_word:
            if has_word2 and found_word_1based == loc.get("word2"):
    ...
    if stripped_records:
        print(
            f"\nRecords requiring non-exact (stripped) match"
            f" ({len(stripped_records)}):"
        )
