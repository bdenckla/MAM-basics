# Initially generated by GitHub Copilot.
"""
HTML document: Documentation template (נוסח).

To regenerate the output HTML, run from the repo root::

    .venv\\Scripts\\python.exe py\\main_authored.py gen-mam-parsed-docs

Output goes to ../MAM-parsed/gh-pages/mpplus_nusach.html.
"""

from author_util import author
from author_util import json_block
from author_util.claim import ClaimCollection
from author import mp_cmn as cmn

_FNAME = "mpplus_nusach.html"
_TITLE = "Documentation template (נוסח)"

_PLAIN_DOC = "mpplain.html"
_PLUS_DOC = "mpplus.html"


def gen_html_file(tdm_ch, claims: ClaimCollection):
    """Generate mpplus_nusach.html in the given output directory."""
    cbody = build_body(claims=claims)
    return author.help_gen_html_file(__file__, tdm_ch, _FNAME, _TITLE, cbody)


def build_body(*, claims: ClaimCollection):
    json_nusach = cmn.emit_claim_by_id(
        claims=claims,
        claim_id="mp.plain.example.nusach",
    )
    json_nusach_plus = cmn.emit_claim_by_id(
        claims=claims,
        claim_id="mp.plus.example.nusach",
    )
    plain_back_link = author.anchor_h("Reading $MAM-parsed-plain", _PLAIN_DOC)
    plus_back_link = author.anchor_h("Reading $MAM-parsed-plus", _PLUS_DOC)
    return [
        author.heading_level_1(["Documentation template (", author.hbo("נוסח"), ")"]),
        author.para(["← Back to ", plain_back_link, " / ", plus_back_link]),
        author.para(
            [
                "Its first parameter is the \u201ctarget\u201d \u2014 what is being documented."
                " The second parameter contains the documentation"
                " (anomalous forms, variant readings, uncertain readings, etc.).",
            ]
        ),
        author.heading_level_2("Plain format example"),
        json_block.json_block_raw_html(json_nusach),
        author.heading_level_2("Plus format example"),
        json_block.json_block_raw_html(json_nusach_plus),
    ]


def _build_body(*, claims: ClaimCollection):
    return build_body(claims=claims)
