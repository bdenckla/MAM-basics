"""Preflight and execute conservative retirement of one Codex worktree.

The ordinary Claude worktree sweep deliberately does not call this module.  A
Codex retirement is a separate, explicitly targeted operation: Codex prepares a
JSON preflight after auditing an ended task, then executes from a separate task or
checkout under the ordinary user token.  Execution re-runs the audit, refuses
drift, preserves every ``.novc`` directory outside the worktree, and uses neither
forced worktree removal nor forced branch deletion.  An ACL-protected directory
left after Git unregisters the worktree is measured and retained, never an
automatic reason to elevate.
"""
...
def _codex_owner_is_explicit(worktree: _Worktree) -> bool:
    codex_root = Path.home() / ".codex" / "worktrees"
    in_codex_root = _inside(worktree.path, codex_root) and not _same_path(
        worktree.path, codex_root
    )
    branch_is_codex = worktree.branch is None or worktree.branch.startswith(
        _CODEX_BRANCH_PREFIXES
    )
    return in_codex_root and branch_is_codex
...
def _content_exists_elsewhere(here: Path, there: Path) -> bool:
    ...
    if not here.is_dir() or here.is_symlink():
        return False
    seen = 0
    try:
...
def _operation_markers(worktree: Path) -> list[str]:
    ...
    for marker in _GIT_OPERATION_MARKERS:
        ...
        if path_text and marker_path.is_file():
...
def _safety_snapshot(worktree_path: Path) -> dict[str, Any]:
    ...
    if not _codex_owner_is_explicit(worktree):
        raise RetirementError(
            "target is not an explicit Codex-owned worktree, or its branch is not Codex-owned"
        )
    if worktree.locked:
    ...
    return {
        ...
        "tracked_novc_citations": _tracked_novc_citations(worktree_path),
    }
...
def prepare_retirement(
    ...
    *,
    ...
    if citations_reviewed and citations and not citation_note:
    ...
    print(f"Codex worktree retirement preflight: {preflight_file}")
    ...
    if citations:
        ...
        for citation in citations:
            print(f"  {citation['path']}:{citation['line']}: {citation['text']}")
    return preflight
...
def execute_retirement(
    preflight_file: Path, *, confirm_task_ended: bool
) -> dict[str, Any]:
    ...
    if preflight.get("kind") != "codex-worktree-retirement-preflight":
        raise RetirementError("file is not a Codex worktree retirement preflight")
    ...
    common_metadata = {
        "schema_version": SCHEMA_VERSION,
        "kind": "codex-worktree-retired-novc",
    ...
    if residue["exists"]:
        ...
        print(
            f"Codex worktree unregistered; residue retained at {residue['path']}: "
        ...
        print(f"removed Codex worktree {worktree}")

    branch = old_snapshot["branch"]
    branch_deleted = False
    if branch is not None:
        if not branch.startswith(_CODEX_BRANCH_PREFIXES):
            raise RetirementError(
                f"refusing non-Codex branch after worktree removal: {branch}"
            )
        exists = _git(
        ...
        if exists.returncode not in (0, 1):
            raise RetirementError(
                f"cannot determine whether Codex branch still exists: {branch}"
            )
        if exists.returncode == 0:
            branch_tip = _git_ok(primary, "rev-parse", branch).strip()
            if branch_tip != old_snapshot["head"]:
                raise RetirementError(
                    f"Codex branch moved after preflight and will not be deleted: {branch}"
                )
            if not _is_ancestor(primary, branch, old_snapshot["default_branch"]):
                raise RetirementError(
                    f"Codex branch is no longer merged into {old_snapshot['default_branch']}"
                )
            ...
            print(f"deleted merged Codex branch {branch}")
        branch_deleted = True
