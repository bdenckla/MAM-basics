...
Every subcommand takes a required --deck naming one of the decks in
ALL_DECKS: what-is-mam, mam-is-a-dataset, the-next-700-bibles.  Each deck's
own module supplies its slides and its images directory, so the paths below
are all derived from the deck rather than given.

Subcommands:
    render-slides
                Render the deck's slide PNGs from its Python source, writing
                them to the deck's images directory.  Name a single SLIDE to
                render just that one instead of the whole deck; --html-only
                generates the interactive HTML and skips the PNG screenshots
                (multi-step slides only).
    make-thumbs
                Generate thumbnails from the slide-*.png images
                render-slides wrote.  --images-dir overrides where they are
                read from, and --thumb-width / --thumb-height the size.
    make-pptx
                Build a .pptx from the slide order in the deck's script.md.
                --script and --output override the two paths, which otherwise
                default to the deck's own script.md and <deck>.pptx beside it.
