...
```
.venv/Scripts/python.exe py/main_list_missing_aleppo_imgs.py          # first 15 missing
.venv/Scripts/python.exe py/main_list_missing_aleppo_imgs.py --all    # all missing
...
.venv/Scripts/python.exe py/main_gen_aleppo_crop_editor.py --all              # all missing
```
...
### Apply crops (one-off scripts in this repo's `.novc/`)

After the user exports JSON from the editor, create a one-off script
(e.g., `.novc/apply_word_crops_batchN.py`, in **this** repo, since it imports this
repo's code) that:
...
# A gitignored throwaway script is the one place a sys.path insert is allowed
# (~/.claude/CLAUDE.md, "No sys.path surgery" -- throwaway scripts are exempt).
...
1. **List missing:**
   ```
   .venv/Scripts/python.exe py/main_list_missing_aleppo_imgs.py
   ```
...
4. **Paste the JSON** into the chat. The assistant creates an apply
   script in this repo's `.novc/`, runs it, and generates the PNGs.
...
5. **Rebuild HTML and verify:**
   ...
   modified files whose blobs were unchanged, git's cached stat sizes being the
   pre-run CRLF ones. `git diff --stat HEAD` reads the blobs. (It also could not
...
- `py/author_boj_util/short_id_etc.py` — `short_id()` extracts the SID from
  a quirkrec.
- `py/author_boj_util/img_util.py` — `get_auto_imgs()` auto-detects
