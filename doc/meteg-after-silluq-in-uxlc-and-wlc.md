...
7. **The plan's precondition holds: the screen's class (v) has 4 members against UXLC 3.9 and 3 against WLC 4.22, in the results the screen's run left and in this run's recount.** On the whole pass class 1 is exactly the screen's class (v) (section 9, item 4). The recount differs from the screen's section 2 only in class (i), one lower, and class (vi), one higher, against each transcription, because MAM-simple changed at 2 Chronicles 26:15 between the two runs; that verse is not a hit either way (section 8).
...
Each column is one comparison of MAM with one transcription, made by the run's script. The passes are defined in section 9, item 3; the aligned pass is this run's addition to the plan.
...
Four throwaway scripts of a Claude session made the figures and this file. They are untracked, and nothing here depends on finding them:

1. **The run** loaded MAM-simple, UXLC 3.9, WLC 4.22 and WLC 4.20 with the screen's loaders, took each verse's verse-final chanted word, and compared the two sides in the pass their letters called for. It classed every hit and counted its syllables with `ben_onsets`, checking MAM's count and stress against Phonetic MAM; recounted the screen's classes; spelled every hit's forms out by Unicode name as a check on the loaders; and raised unless the calibration held. Its results are every figure of sections 1 to 5 and 7 and the recount of section 8.
2. **The links** came from running `py/main_verse_links.py` for each class 1 member (section 6).
3. **The MAM-simple check** compared each of the changed lines of `MAM-simple/json-vtrad-bhs` between the screen's commit and this run's, and gave the screen's class of 2 Chronicles 26:15 against each transcription at both (section 8).
4. **The writer** wrote this file from the other three scripts' results and from UXLC's change record for 1 Kings 14:14. Before writing, it asserted every count, list and position the prose states against the data; after building, it checked every line with `uni_denorm.has_std_mark_order` and for a first strong character that is Hebrew, and checked that nothing in the file points into a `.novc` directory. `py/tests/test_prose_mark_order.py` checks the file's mark order again as part of the suite.

The figures were measured against MAM-basics at `5f996d0e`, that is, MAM-simple, `in/UXLC-39`, `out/wlc422-u` and `out/wlc420-u` as committed there, and against Phonetic MAM in MAM-private at `ecab7264`. What is not expected to change: the calibration and the class 1 list. A re-run after MAM-simple, UXLC or WLC moves is a new measurement, and a hit appearing or vanishing is a finding rather than noise.
