...
**One commit in codex-index-aleppo and ten here** (this read "five here" from `4682adf` until the
2026-08-22 review's follow-up; `git rev-list --count ef5525d^..72a4629^` is **10**, the five
that `4682adf` could not know of being `6621db4`, `c3a6287`, `b594c72`, `f6e7ad2` and `60aabf5`,
which followed it — the same correction-then-falsified pattern the sentence below already
records for itself). `a171dd4` there and `ef5525d` here are the two
halves of the one edit item 4 of the programme's Order hands this step, and are the only code
change this phase made. The other nine here are all prose: `eca7f14` backfilled `ef5525d`'s own
hash, `0ea8d3e` recorded the CRLF defect a verification run turned up in codex-index-cam1753,
`1e36d56` corrected the provenance claim below, `4682adf` repaired the sentence you are reading,
which said "one commit in each of two repos" and was true for about an hour, and the five named
in the parenthesis above are further provenance corrections, all prose in `doc/`.
...
### Preconditions — every one matched, and MAM-basics is ten commits further on (this heading said "four prose commits" until the 2026-08-22 review's follow-up)

MAM-basics at **`60aabf5`**, not the `90487af` Phase 0's record names. The ten commits between
(`git rev-list --count 90487af..60aabf5`; this read "the four commits between" and then listed
seven hashes until the 2026-08-22 review's follow-up) are Phase 0's own `ef5525d`, its
`eca7f14` and `0ea8d3e`, and that phase's provenance corrections — `1e36d56`, `4682adf`,
`6621db4`, `c3a6287`, `b594c72`, `f6e7ad2`, `60aabf5` — the nine after `ef5525d` all prose in
`doc/`, none touching anything this phase reads.
...
Landed in MAM-basics as one commit: nineteen files added and four modified, the fourth being
this plan itself, carrying this record (this said "three modified" until the 2026-08-26 review;
`git diff-tree -r --name-status fe6cef2` has 4 M entries). **Nothing was owed
...
`py/py_ac_loc/mam_xml_verses.py` are **one tool with drift, not two tools** — 32 diff lines
(28 insertions + 4 deletions; this said "43" until the 2026-08-26 review), 177 lines against
201, most of it docstring. Both read the same three books, and **the MAM-XML snapshots
...
### The doc repointing is twenty-six sites in five files, and three of them were wrong before this programme began

(This heading said "four files" until the 2026-08-22 review's follow-up; the count in the next
paragraph names five, and `git -C ..\codex-index-aleppo show 078b74d --name-status` has 5 M
entries — `CLAUDE.md`, `README.md`, `doc/aleppo-line-breaks.md`, `doc/ocr-with-kraken.md`,
`doc/reading-mam-simple.md`.)
...
it μY.** Measured across MAM-basics 2026-08-22: **58 occurrences of μY against 3 of μC** (this
read "57" until the 2026-08-22 review's follow-up; `git grep -oI 'μY' 492057d^ | wc -l` is 58,
as is the sum of `git grep -c 'μY' 492057d^ -- . ':!*.png'`, the two PNGs whose bytes happen to
match being excluded; the 3 of μC re-count at the same commit), and
...
| `.claude/settings.json` | three of its ten permission globs name this repo's own `.venv` python, which no longer runs here | the other seven are still live: `Bash(git *)`, `Bash(cd **/* && git *)`, three `gh issue` globs, a `Bash(**/.venv/…)` glob and a `Read()` glob over the Yeivin scans folder. (This row said "six … name a `.venv` python" and "four … still live: … two `gh issue` globs" until the 2026-08-26 review; re-derive with `git -C ..\codex-index-aleppo show "3003a06^:.claude/settings.json"`.) |
...
`py/main_vendoring.py --all` then rewrote the four generated files (this sentence said "the
three tracked artifacts" until the 2026-08-26 review — its own next sentence already counted
four): **`doc/vendoring-inventory.md` 18 rows/112 files → 12 rows/97 files**, and
`out/vendoring_compare_out.txt` **112** data rows → 97 (this said "105 rows → 97" until the
2026-08-26 review; 105 was the uncommitted stale-state regeneration at `ea1f035` the next
section describes, and the tracked file went 114 lines → 99, 2 of them headers). No
...
### Ben said yes to all five, and doing them found a SIXTH site — in this repo

**Ben, 2026-08-22: "yes, do all five repointings."** They landed as
**`b1e1a2d`** (holman-ketiv-qere), **`81e036b`** (book-of-job), **`292e7a7`** (UXLC-utils) and
**`fe175c3`** (MAM-private), plus the sixth below in this repo's own commit.

**Doing them turned up more than the five sites named**, in three different ways, and the pattern
is worth the paragraph: **a stale citation is rarely alone in its paragraph.**

- **UXLC-utils had a second site with the same fact.** `doc/clc-design.md:286` also said the
  sparse copy is "refreshed by that repo's `main_update_vendored_files.py`", so `292e7a7` fixed
  two files rather than one. Item 2's fact had **three** homes in the end — this repo's policy
  comment, `shared-with-codex-index-leningrad.md`, and that line — where the plan named none.
- **THE SIXTH SITE IS IN THIS REPO, and Phase 6 should have caught it.**
  `py/main_uxlc_estimate_atom_loc.py`'s docstring said its word-finding is kept in sync with "the
  sibling repos: codex-index-aleppo — `py/py_ac_word_image_helper/linebreak_search.py`,
  codex-index-cam1753 — `py_cam1753_word_image/linebreak_search.py`". Both modules are **this
  repo's** since Phase 3, so keeping the three in sync is a same-repo job now. **Phase 6 swept the
  three codex-index repos' tracked files and not the code that had just landed here**, which is
  the gap: a move leaves stale citations on BOTH sides, and the arriving side is the one whose
  sweep nobody prescribes. Re-measured after finding it, exactly **one** file in this repo's `py/`
  cites a codex-index repo as holding code, so the gap cost one docstring rather than a class of
  them.
- **Three things in mgketer were wrong for reasons the repointing forced into view.** The
  `PYTHONUTF8=1` prefix on the Aleppo command, which the entry point's own
  `force_utf8_io` in `main()` makes unnecessary and which the user-level `CLAUDE.md` forbids on
  tracked scripts; **the `../` depth, which was wrong before this programme began** — that tree
  sits at `MAM-private/mgketer/`, so a GitRepos sibling is TWO levels up, the depth
  `py_ac_word_image_helper/codex_page.py` spells as `MGKETER_ROOT.parent.parent`, and both
  commands spelled it with one; and a `book_id` pointer naming `py/my_tanakh_book_names.py` and
  an `ALL_BOOK_IDS` tuple, **neither of which exists anywhere in that tree**, against the real
  `mb_cmn/bib_locales.py` and `ALL_BK39_IDS` here.

**Two nearby stalenesses were reported rather than fixed, because they belong to other plans.**
`UXLC-utils/doc/clc-design.md:251` says wlc-utils vendors `in/UXLC-*` "refreshed by its
`main_update_vendored_files.py`", where wlc-utils has held zero tracked `.py` since its own
evacuation and `py/main_wlc_vendor_uxlc.py` here does that job; and
`book-of-job/py_ac_loc/codex-index/codex-index-provenance.md` names the repo `bdenckla/codex-index`,
gone in the 2026-03-28 rename. The second is a provenance question rather than a repointing —
saying where that JSON came from is not the same as saying where a script lives.

**`doc/review-findings-2026-08-22.md:316` had independently found two of the five**, holman's
`CLAUDE.md:104` and `:91–92`, filed under "stale-by-later-work, accurate when written". That
review record is left as written. It also names one this phase did not touch: holman's
`README.md:328` calls the NFC scope "one of three" where the test now has seven.
