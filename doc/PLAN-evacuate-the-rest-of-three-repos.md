...
**For scale against the precedent:** wlc-utils' move carried 284 files / 50.0 MB and left 155
stubs. This is about four times its file count in the `gh-pages/` half alone, and the 737 files
outside `gh-pages/` have no counterpart in it at all — wlc-utils' `out/` 193, `in/` 135, `doc/` 6
and `data/` 1 came to 335, which is less than half of what these three repos hold outside their
published trees.
...
**Recommendation:** add a row set per arriving tree to `DATA-LICENSES.md`, modelled on the three
`gh-pages/wlc/…/img/` rows that already say *"each rights holder's; no grant is made or implied
here"*; leave each emptied repo without a `LICENSE`, as it is today. **The question for Ben is the
...
`py/main_wlc_vendor_uxlc.py` (twice), plus **eight literal-string sites in
`py/tests/test_verify_table_notes_in_uxlc.py`**, which builds `tmp_dir / "UXLC-utils" / "in" /
"UXLC-39"` and passes `uxlc_utils_path=`. A repoint that follows `uxlc_paths.py` alone misses all
twelve.
...
- **`all-repos.code-workspace`'s three entries.** Both the UXLC-utils plan and the book-of-job
  ...
  expires here.** Phase 6 disposes of it. Note that wlc-utils is still listed, deliberately.
...
2026-08-22: `py/main_0_mega.py`'s `_STEPS` holds **34** steps and **not one of them is a
book-of-job, holman-ketiv-qere or UXLC-utils generator**. Re-establish with
...
including the pattern that a directory of manuscript crops gets its own row saying no grant is made
or implied. **Do this before Step 3**, so the statement lands with the files rather than after
...
not merely on the page, which is the acceptance test for the JavaScript half specifically.
...
decision, 2026-08-22**, given while this plan was being written and departing from the wlc-utils
precedent, whose clone is still on the disk at `C:\Users\BenDe\GitRepos\wlc-utils`.
...
identical bytes. Run the **other two repos'** oracles as well, since `holman-ketiv-qere`'s
...
**What exists today**, measured 2026-08-22: `py/main_wlc_redirect_stubs.py` (116 lines) plus
`py/wlc_redirect/` — `stubs.py` (297), `build.py` (52), `check.py` (52), 517 lines in total. The
wlc-specific facts are **four module-level constants in `stubs.py`** — `NEW_SITE`,
`OLD_PATH_PREFIX`, `_PAGES_PREFIX` and the repo name inside `wlc_utils_pages_dir` — plus the prose
...
and in `py/wlc_redirect/stubs.py`'s own docstring. **Recommendation: rename to
...
true when they ran. **Put the rename to Ben with Decision A**, since both bear on what the tool is
called.

**Verify:** `build --out <scratch>` for the wlc rows still produces exactly the 155 files
committed in wlc-utils, byte-identical — `git diff --no-index` between the scratch tree and
`../wlc-utils/gh-pages` is the check, and it is expected to be empty; `check` passes against
wlc-utils' committed tree; `build` for a repo whose tree has not landed produces an empty set and
`check` says so rather than passing vacuously; `py/main_test.py` green; `black` and `ruff` clean on
the files touched.
...
1. **The docx has spaces in its name and `git ls-files` does NOT quote it**, being pure ASCII. The
   ...
   pages and misses the 20.6 MB document.** Use `-z`.
...
`py/main_extract_docx_and_render_table.py`, `py/main_lenin_vendor_uxlc.py` and
`py/main_wlc_vendor_uxlc.py`, plus **eight literal-string sites in
`py/tests/test_verify_table_notes_in_uxlc.py`**. A repoint following `uxlc_paths.py` alone misses
twelve sites. **`uxlc_paths.py` also has no `DATA_REPO_NAME` to grep for.**
...
**Trap 6 — this repo's Step 6 is the one that breaks other things, so run the other two repos'
oracles after it.** Removing the UXLC-utils clone takes away a directory that
...
4. **The `hebrew-prose` skill**, live at `C:\Users\BenDe\.claude\skills\hebrew-prose\` and tracked
   ...
   `references/terminology.md` names book-of-job and UXLC-utils. **Edit both copies and verify
...
8. **`py/repo_util/check_repo_standards.py`** discusses these repos in prose. Follow the convention
   ...
   written.
9. **`MAM-private/masorah-books/doc/migration-checklist.md`** names three book-of-job paths, one of
   them (`../book-of-job/pyauthor_qr/qr_1413.py`) **already stale** since that tree moved
   2026-08-19, and warns that they are deep-linked. One commit in MAM-private.
