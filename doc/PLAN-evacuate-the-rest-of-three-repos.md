...
**Assessed 2026-09-02 while paused, four decisions taken, and STILL PAUSED — nothing here is to
run until Ben says so.** Ben, 2026-09-02: *"Although the 'forest of worktrees' technique has shown
promise, nothing beats a single worktree."* That sentence is now the motivation of record for this
plan and for the third stage that follows it: every public sibling that MAM-basics' code, tests or
pipeline reaches at run time is one more worktree a task needs, and a total evacuation removes the
sibling. The same day Ben asked for an assessment of where the evacuation stands and where it goes
...
session re-measures every figure the assessment corrected** — the assessment was itself overtaken
twice in the hours it was being written (its items 3 and 10), which is the standing lesson about
figures in these plans arriving one more time.
...
Measured on 2026-09-02 against MAM-basics `18d17e8`, holman-ketiv-qere `48ee065`, book-of-job
`d09b966` and UXLC-utils `b7b4eb9`, all four clean and in sync with origin. Each item says what
the resuming session does about it; where the fix was one sentence it was made in place the same
day and the item says so.

1. **The landing page is generated already, and the derived subtree list this plan prescribes was
   tried and deleted by Ben.** `gh-pages/index.html` has opened `Do not edit by hand. This file
   was generated by MAM-basics/py/author_site/site_index.py` since `13d1d31` (2026-08-31), when
   document-index's README became the page, as Ben's authored document index
   (`py/author_site/site_data.py`). A derived "Pages published from this repository" section
   existed for under an hour that day and Ben deleted it (`2b0e67d`), disliking it. So Phase 3's
   subsection "The generated landing page rides with this phase's Land step" and the programme's
   "Decision — the site's landing page becomes generated" are SPENT — both marked in place — and
   **no per-subtree list is wanted**. What a lane owes the page instead is a repoint of the page's
   own citations of that lane's repo: `py/author_site/site_data.py` links
   `https://bdenckla.github.io/book-of-job/jobn/job2_main_article.html`, a citation Ben controls
   and Phase 4's, to become the relative `book-of-job/jobn/job2_main_article.html`;
   `py/tests/test_site_index_links.py` lints the page. `DATA-LICENSES.md`'s `gh-pages/index.html`
   row was rewritten on 2026-08-31, so Phase 3's "that row expires" item is spent too.
2. **The Holman mailbox is per-machine, there are now TWO of them, and "13 messages that only the
...
3. **holman-ketiv-qere gained a third body of Holman's work on 2026-09-02, with a stricter privacy
   boundary, and its oracle is seven commands plus one.** MAM-basics `fce0375` and
   holman-ketiv-qere `48ee065` ("Record Holman's 34 suggested corrections to MAM"):
   `py/main_ingest_mam_suggestions.py`, with `py/hkq_cmn/mam_suggestion_extract.py`,
   `verify_mam_suggestions.py`, `xlsx_xml_utils.py` and `holman_email_common.py`; three new
   accessors in `py/hkq_paths.py` (`mam_eml_dir`, `mam_suggestion_img_dir`,
   `mam_suggestions_json_path`); 34 crops under `gh-pages/mam_img/` and
   `docs-not-served/mam_suggestions.json`. Phase 3's `CLAUDE.md` triage gains a fifth section, a
   CARRY, written in below; the oracle table's holman row and Phase 3's copied oracle block name
   the seven. The holman Scale row is now **385 tracked / 60.61 MB, `gh-pages/` 335 / 37.97 MB /
   6 HTML, 50 outside** (348 / 58.1 / 300 / 35.5 / 6 / 48 on 2026-08-22), `docs-not-served/` is 5
   files, and the Scale table is left as measured, per its own rule; re-measure at resumption.
4. **Nine citations in this file pointed at four plan files deleted on 2026-08-29** (`f6173fe`,
   "Delete seven spent plans; keep the two the paused plan leans on"):
   `PLAN-evacuate-python-from-holman-ketiv-qere.md` (the sole cited source of holman-ketiv-qere's
   oracle row), `PLAN-evacuate-python-from-book-of-job.md` (five sites),
   `PLAN-evacuate-python-from-UXLC-utils.md` (two) and `PLAN-evacuate-python-from-wlc-utils.md`
   (one). Each is marked in place with the `git show f6173fe^:doc/<file>` that re-establishes it,
   and the holman oracle block is copied into Phase 3, because its only other copy is
   holman-ketiv-qere's `CLAUDE.md`, which Step 5 shrinks and Step 6 removes from the disk.
5. **The suite baseline moved to 971 passed / 5 skipped / 65 subtests** on 2026-09-01
   (`doc/review-findings-2026-09-01.md`), from Phase 0's 941 / 5 / 59, and test files have landed
   since; re-measure at Phase 1 rather than carrying either figure.
6. **`all-repos.code-workspace` lists 18 folders, not 19** — `c0b3195` de-listed document-index,
   `af77984` added `../Taamey_D`, `9086da4` removed `../ArtScroll`. Phase 6's item 7 is corrected
   in place: 18 → 15 for this plan, → 12 after the trio, → 11 after diffable-pointed-hebrew.
7. **Decision C is superseded by Decision F** — one top-level directory per evacuated repo,
...
9. **The stub generator did not move**, which is the one place no drift is good news:
   `py/main_wlc_redirect_stubs.py`, `py/wlc_redirect/` and `in/wlc_redirect_pages.json` (154
   paths) are as Phase 2 describes them, still one repo, and `py/tests/test_wlc_redirect_manifest.py`
   has been green on every suite run since.
10. **What else moved, and what did not.** MAM-basics `d095871` → `18d17e8`, 2,313 tracked /
    273 MB before `fce0375`, `gh-pages/` 287 / 49.97 MB / 156 HTML — some 185 commits, of which
    only `297f7e5` (the two qere-search modules stop resolving holman-ketiv-qere at import time),
    the landing-page pair and item 3's `fce0375` touch this plan's subject. book-of-job `3f096b9`
    → `d09b966`, one commit repointing a link, every Scale figure reproducing; UXLC-utils still
    `b7b4eb9`. The docx, `table_data.json` and the whole extraction chain are intact; book-of-job's
    `py_ac_loc/` and all four of Decision E's UXLC-utils files are present; `.gitattributes` in
    all four repos is as Phase 1's table has it, so Phase 1 is verification-only, confirmed against
    the live files; no source repo has a `.venv` any more, though holman-ketiv-qere's `CLAUDE.md`
    still names one. The sibling-reach survey behind Decision G lives in the programme's
    third-stage section and is not repeated here.
...
than a reopening of `PLAN-evacuate-python-from-wlc-utils.md` (deleted 2026-08-29 by `f6173fe`;
in git history), and the programme says explicitly
...
**SUPERSEDED 2026-09-02 by Decision F below — one top-level directory per evacuated repo,
`holman/` here.** Everything under this heading is left as the record of what was decided on
2026-08-23 and why; none of it is to be executed. What survives of it is carried by Decision F: the
`data/` particular becomes `holman/data/`, `halve.md` stays dropped, and the `lci_recs.json` pair
question moves to Phase 5 with the rest of trap 3.
...
`PLAN-evacuate-python-from-book-of-job.md` (deleted 2026-08-29 by `f6173fe`; in git history)
records Ben deleting the equivalent file in that repo —
...
### Decision F — where the non-`gh-pages` tree lands: one top-level directory per evacuated repo — **DECIDED 2026-09-02, superseding Decision C**

**Ben, 2026-09-02: `holman/` for holman-ketiv-qere, and `aleppo/`, `leningrad/` and `cam1753/`
for the codex-index trio** — each repo's whole non-`gh-pages` tree moved under that one directory
as a pure prefix (`holman/emails/`, `holman/docs-not-served/`, `holman/assets/`, `holman/io/`,
`holman/data/`, `holman/out/`, `holman/doc/`), while the published subtree stays at
`gh-pages/<subtree>/` per Decision A. Put to him with the recommendation, against two alternatives:
Decision C as decided (five new top-level directories for holman, and the trio's `aleppo-pages/`,
`cam1753-spreads/` and the rest at MAM-basics' top level), and folding into MAM-basics' own shape
(`in/holman/`, `out/holman/`, `doc/holman-*.md`), which classifies every arriving file as input or
output. He took the recommendation.

Why it was recommended: a pure prefix move keeps every relative path inside the tree valid and
makes every accessor a one-line change — `paths.repo_root() / "holman" / …` where
`hkq_data_root() / …` stood, the shape `hkq_paths.py` was designed to be changed in — and it is
the convention MAM-private already uses, one top-level directory per evacuated source repo, so
both destination repos read the same way. The two accessors Step 3 must not sweep are unchanged:
...
**Ben, 2026-09-02: holman-ketiv-qere, then codex-index-leningrad, codex-index-aleppo and
...
no Pages, no stubs) and its evacuation retires `UXLC-utils-sparse/` and
`py/main_lenin_vendor_uxlc.py`, which dissolves Phase 5's trap 4 and the codex-index-leningrad
commit that trap owes, before Phase 5 runs; codex-index-aleppo and codex-index-cam1753 each remove
two suite-and-lint dependencies for at most three stubs; book-of-job is the largest move but the
simplest; UXLC-utils stays last of the six as the entangled one, and after book-of-job because
`main_map_changes_to_book_of_job.py` reads book-of-job's clone; diffable-pointed-hebrew closes
because it is small and touches nothing the others touch. **A session picking up either plan reads
the other's Status first**, so that no phase runs out of this order.
...
|---|---|---|---|
| book-of-job | `py/main_gen_misc_authored_english_documents.py`, alone | **183 of 701** rewritten; **518 written by no program**, being essentially the 515 PNGs | `PLAN-evacuate-python-from-book-of-job.md` §"The oracle: `main_gen_misc_authored_english_documents.py`, alone" — **deleted 2026-08-29 (`f6173fe`); `git show f6173fe^:doc/PLAN-evacuate-python-from-book-of-job.md` re-establishes it** |
| holman-ketiv-qere | **seven commands, not one, since 2026-09-02** (six until `fce0375`): `main_extract_docx_and_render_table.py`, `main_ingest_uxlc_emails.py`, `main_estimate_uxlc_locations.py`, `main_render_uxlc_corrections.py`, `main_ingest_mam_suggestions.py`, `main_search_holam_he_qere.py`, `main_search_final_hiriq_verse_text.py`; plus `main_just_render_table.py`, which re-renders from the extracted JSON without the docx | 335 artifacts, 175 rewritten and 160 untouched at that plan's Phase 3; `aacd3ee` and `48ee065` have since added two `emails/` files, 35 PNGs and one JSON — re-measure | its Python plan's "Regenerating everything is six commands, not one" block — **that plan was deleted 2026-08-29 (`f6173fe`); the block is copied into Phase 3 below, from holman-ketiv-qere's `CLAUDE.md` §"This repo contains no Python", its only other copy** |
| UXLC-utils | `py/main_uxlc_mega.py`, `py/main_clc.py`, `py/main_map_changes_to_book_of_job.py` | **214**; at that plan's Phase 6, **127 rewritten and 87 not** | `PLAN-evacuate-python-from-UXLC-utils.md` §"The oracle ran as specified and passed" — **deleted 2026-08-29 (`f6173fe`); `git show f6173fe^:doc/PLAN-evacuate-python-from-UXLC-utils.md` re-establishes it** |
...
wholesale, and `PLAN-evacuate-python-from-UXLC-utils.md` (deleted 2026-08-29 by `f6173fe`; in
git history) records that same warning at the point
...
`doc/PLAN-evacuate-python-from-book-of-job.md` (deleted 2026-08-29 by `f6173fe`; in git
history), which describe the URLs. **holman-ketiv-qere's
...
## Phase 2 — Generalize the redirect-stub generator to a table of four — DONE 2026-09-03
...
### Execution record — Phase 2, 2026-09-03

Began at MAM-basics `c1cd492`, the Phase 1 commit, with its main checkout clean and with
book-of-job `fea3acd`, holman-ketiv-qere `7a25b34` and UXLC-utils `a2768f4` each clean on
`main`. The Python-evacuation programme's status table still has every row DONE. The latest
MAM-basics Pages deploy, `33752117742`, succeeded, and
`https://bdenckla.github.io/MAM-basics/wlc/index.html` returned HTTP 200. The temporary
depth-1 wlc-utils clone began clean at `8250b69`; it was not added to either workspace file.

**The generator is now a table, not a wlc-utils special case.**
`py/main_redirect_stubs.py` and `py/redirect_stubs/` replace their wlc-named predecessors.
`RedirectRepo` holds the source repository, MAM-basics subtree, old URL prefix, manifest path
and clone URL; the table has the wlc-utils row alone until each later lane captures a frozen
manifest. `--repo` selects a row while preserving wlc-utils as the one-row default. The
manifest test is now `test_redirect_manifest.py`, parametrized over the table. `target_url()`
percent-encodes a page path with `quote(..., safe="/")`, which changes nothing for wlc-utils'
ASCII paths and is needed for Holman's two future `#`-and-Hebrew page names. A constructed row
with no manifest raised `FileNotFoundError`, rather than building or checking an empty set.

**The byte-identity oracle passed before the rename.** A fresh `build --out` produced 155
files, and `git diff --no-index` against wlc-utils' committed `gh-pages/` tree was empty;
the generated-tree `check` passed too. After the rename, each of the same 155 files differed
by exactly one line, changing only its `GENERATED by` comment. `build --publish` then wrote
those 155 comment changes into the clone, and `check` passed against its committed tree.
`dbb5d49` in wlc-utils records the 155 generated comments and the redirect-host `CLAUDE.md`
repoint: the command now names `main_redirect_stubs.py`, and the instructions now state that
the URL set is frozen at `in/wlc_redirect_pages.json` rather than derived from the live tree.

**The recorded `65 subtests` did not reproduce.** `py/main_test.py` collected 976 tests and
...
**The oracle, copied here on 2026-09-02 from holman-ketiv-qere's `CLAUDE.md` §"This repo contains
...
1. `py/main_extract_docx_and_render_table.py` — reads the tracked docx; writes
   ...
   verification against `../MAM-parsed/plus/*.json` is part of it. **Changes shape at Step 3**
   (Decision B's sub-question): the extraction half retires and the command is renamed for what
   it still does.
2. `py/main_ingest_uxlc_emails.py` — writes `emails/<key>.txt` and `emails/<key>.json`, and each
   attached PNG into `gh-pages/uxlc_img/`; needs the untracked mailbox `.novc/eml/`.
3. `py/main_estimate_uxlc_locations.py` — writes `data/uxlc_atom_locations.json` and
   `data/uxlc_standard_atoms.json`; needs the UXLC-utils clone until Phase 5's Step 3 repoints it.
4. `py/main_render_uxlc_corrections.py` — writes `gh-pages/uxlc_corrections.html`,
   `docs-not-served/uxlc_corrections.json`, and the `gh-pages/` copies of
   `assets/uxlc_corrections.css` and `assets/uxlc_corrections.js`; needs only what is tracked.
5. `py/main_ingest_mam_suggestions.py` (since 2026-09-02, `fce0375`) — writes
   `docs-not-served/mam_suggestions.json` and the crops under `gh-pages/mam_img/`; needs the
   second untracked mailbox `.novc/eml-mam/`; verification against `../MAM-parsed/plus/*.json` is
   part of it.
6. `py/main_search_holam_he_qere.py` — writes `out/holam_he_qere_report.json`.
7. `py/main_search_final_hiriq_verse_text.py` — writes `out/final_hiriq_verse_text_report.json`.

An eighth, `py/main_just_render_table.py`, re-renders the report pages from an existing
`table_data.json` without the docx, and after Step 3 it is the first command's successor in all
but name. `table_data.json` holds exactly 77 rows, a fixed scope, so a run that changes that
number is a failure; `mam_suggestions.json`'s count is not fixed — Holman sends more — so a
changed count there is a finding. **Not everything under the generated directories is
generated**: 160 of the 335 artifacts were untouched by a full run on 2026-08-18 — the 154 images
under `gh-pages/img/` (write-once, `export_images` raising rather than overwrite), `gh-pages/index.html`,
the two JC3 pages, `gh-pages/woff2/Taamey_D.woff2`, `docs-not-served/table_data_fields.md` and
`io/table_row_github_issues.json`, which has a writer of its own,
`main_just_render_table.py --update-issue-metadata`, reading the holman-ketiv-qere tracker by
name. After Step 3 that set grows by everything the docx cord-cut turns into source data.
...
**The two mailboxes are this phase's untracked move, and they are per-machine.** (Rewritten
2026-09-02; until then this paragraph said one mailbox of 13 messages that only the clone held.)
`main_ingest_uxlc_emails` reads `holman-ketiv-qere/.novc/eml/` through `hkq_paths.eml_dir()`, and
`main_ingest_mam_suggestions` reads `.novc/eml-mam/` through `hkq_paths.mam_eml_dir()`. Both are
untracked and both are per-machine — `py/main_ingest_uxlc_emails.py`'s docstring: *"a machine
holds whichever messages were forwarded to it, which may be one. Ingesting a partial mailbox is
...
--porcelain --ignored` still accounts for `.novc/` before the clone goes; what it protects is the
machine's ability to re-ingest, not the site.

**This phase's `CLAUDE.md` triage — five sections, three of them carried** (four and two until
2026-09-02, when `48ee065` added the fifth). Per Step 5's triage rule. holman-ketiv-qere's
`CLAUDE.md` was 124 lines on 2026-08-24 and 160 on 2026-09-02; re-read it rather than trusting
this list.
...
4. **§"This repo contains no Python. Its generators live in `../MAM-basics/py/`" — nothing to
   carry.** Step 5's own prescription for the shrunken file replaces it. Its seven-command block
   is already copied into this phase (above), so nothing is lost with it.
5. **§"The MAM suggestions keep a STRICTER boundary, and it is not the one above" — CARRY.**
   Added 2026-09-02, a subsection of section 1 since `48ee065`. Ben's instruction of 2026-09-02:
   nothing Ben Denckla or Seth (Avi) Kadish wrote is stored in any explicit form — the extract
   keeps a reference, the two forms Holman compares, his one-line description and recommendation,
   and the message's subject, date and sender display name, and **no message body at all**; the
   UXLC ingest's looser redaction rule is not to be extended to these messages because the two
   ingests sit beside each other. `py/hkq_cmn/mam_suggestion_extract.py`'s module docstring states
   the boundary and enforces it structurally (only `SUGGESTION_SENDER_NAME`'s messages
   contribute). It governs `docs-not-served/mam_suggestions.json`, which Step 1 lands, so it goes
   wherever section 1's public-address rule goes: the two are one boundary at two strictnesses,
   and a reader adding a field to either derivative needs both in one place.

### The generated landing page rides with this phase's Land step — SPENT 2026-09-02

**SPENT — see the 2026-09-02 assessment's item 1 under Status.** `gh-pages/index.html` has been
generated by `py/author_site/site_index.py` since 2026-08-31, as Ben's authored document index;
the derived per-subtree list this subsection prescribes existed for under an hour that day and Ben
deleted it. Nothing below is to be built. What a lane owes the page is the repoint of the page's
own citations of that lane's repo, and `DATA-LICENSES.md`'s row was rewritten the same day. The
text below is kept as the record of what was decided on 2026-08-22.
...
3. **§"Which entry point writes what" — CARRY, REWRITTEN.** All five entry points are MAM-basics'
   ...
   `doc/PLAN-evacuate-python-from-book-of-job.md` (deleted 2026-08-29 by `f6173fe`; in git
   history) holds the same table; check whether that record
...
### Execution record — book-of-job, 2026-09-03

Step 1 landed the 694-file published tree in `gh-pages/book-of-job/`, seven JSON
files in `book-of-job/out/`, and two procedures in `book-of-job/doc/`, in
MAM-basics `050c7bb`. SHA-256 verification found every copied file identical to
the source. Step 2 added the five scoped licence rows in `fdf8f3e`; the
MAM-basics deployment served the landing page, two nested pages, and an Aleppo
crop with HTTP 200.

Step 3, MAM-basics `8a04ad5`, made the Book-of-Job paths local to MAM-basics,
corrected the stale WLC-a-notes URL, and repaired `check_all.py`'s redirected
UTF-8 output. The Book-of-Job oracle changed only that known URL; the UXLC
oracle had no Git-content diff; and all 701 source-artifact timestamps were
unchanged. The full MAM-basics suite passed 972 / 5 / 65, Ruff was clean, and
the Book-of-Job check passed all seven checks. The source `py_ac_loc/` tree was
not copied, as Decision D requires.

Step 4 froze the 175 published HTML paths in MAM-basics `dc6442f` and created
the source stubs in `book-of-job` `138db11`. Ben authorized the required
MAM-private scope after the citation sweep found seven controlled mgketer links;
MAM-private `0f37fe3` repointed those links and regenerated the Job report. The
same regeneration exposed an already-landed MAM `t451` update in Daniel, so
MAM-private `8360ef2` records that unrelated report regeneration separately.
The source Pages deployment for `138db11` succeeded, and the final source
deployment for Step 5, run `33781252252`, also succeeded. A live published-page
stub returned HTTP 200 with its MAM-basics target; the live catch-all returned
HTTP 404 with its MAM-basics target. The redirect checker reports 175 stubs and
`404.html`, all correct.

Step 5 carried the four required instructions into MAM-basics `058f6b0`, removed
`../book-of-job` from `all-repos.code-workspace`, and rewrote the two retained
reading procedures for MAM-basics. Source `c8a6bbf` then removed the 85 approved
`doc/`, `out/`, and `py_ac_loc/` files, leaving 176 HTML redirect files and five
metadata files. The source README and `CLAUDE.md` now describe the redirect host.
...
### Execution record — Phase 5, Steps 1–5, 2026-09-03
...
The 27 UXLC-utils paths that first appeared modified had no content diff: every
path's raw Git blob hash matched both the index blob hash and the filtered Git
hash. `git update-index --refresh` retained one stale stat-cache marker even
after the exact metadata-only staging Ben authorized. That stale marker was an
intended source-file deletion at Step 5 and did not represent uncommitted content.

Step 4 froze the 91-page redirect set in MAM-basics `8c42a3f4`, added the
UXLC-utils redirect-table row, and passed the focused manifest test (4 passed).
The source commit `f9794b8` replaced all 91 published pages with generated
stubs and added the generated `404.html`; no source assets changed in that
commit. Its successful Pages deployment `33798546895` served a known redirect
with HTTP 200 and the catch-all with HTTP 404.

Step 5 removed the UXLC-utils workspace entry and committed source commit
`2745c65`. Source commit `2745c65` removes 684 files and retains 97 files: 92
generated HTML stubs, `.github/workflows/pages.yml`, `.gitattributes`,
`.gitignore`, `README.md`, and `CLAUDE.md`. The staged inventory check found
exactly 684 deletions and two documentation edits before the commit. Pages
deployment `33799858167` succeeded; `clc/Genesis.html` served its generated
...
7. **`all-repos.code-workspace` — the three entries must GO, and Step 6 is what forces it.**
   ...
   **Remove each entry in its own repo's lane, in the same commit as that repo's Step 5**, so the
   ...
   `grep -c '"path":' all-repos.code-workspace`. **So this plan takes it to 16 — corrected
   2026-09-02: the file listed 18 that day (document-index and `../ArtScroll` out, `../Taamey_D`
   in), so this plan takes it to 15, the third stage's trio to 12, and diffable-pointed-hebrew to
   11.** Re-run that command rather than assuming the starting point.
