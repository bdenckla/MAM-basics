...
**Two riders travel with this decision, both from Phase 2's "Naming" paragraphs**, which put them
to Ben alongside Decision A because all three bear on what the tool is called: whether to rename
`py/main_wlc_redirect_stubs.py` and `py/wlc_redirect/` (recommended: `py/main_redirect_stubs.py`
and `py/redirect_stubs/`); and, if renamed, whether wlc-utils' 155 committed stubs, whose
`GENERATED by` comment would then name a retired file, are left as they are or republished once
(recommended: republished once). Record both answers here with their dates.
...
**The rename then leaves a question, and it goes to Ben with the rename itself.** After it, all
155 committed files in wlc-utils name a generator that no longer exists at that path, under a
comment that tells the reader to run it. Two answers: **leave them** — the stubs work, the comment
misdirects a reader to a retired file; or **republish them once**, `build --publish` into the
clone this phase has on disk anyway, one commit in wlc-utils, whose diff is exactly one comment
line in each of 155 files — which is also a second proof that the generalization changed nothing
else. **Recommendation: republish once**, on that last ground; the plan's "publishing is a
never-event" is about needing to, and a one-line diff over files already proven identical is the
cheapest publish there will ever be. Whichever Ben chooses, record it beside Decision A.
...
checks (nothing unpushed, nothing untracked). Then: `build --out <scratch>` for the wlc row
