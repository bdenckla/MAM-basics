...
third stage has completed Phases 0–2 as of 2026-09-04
...
diffable-pointed-hebrew, total evacuation". On 2026-09-03 its Phase 0 wrote
`doc/PLAN-evacuate-the-codex-index-trio-and-diffable-pointed-hebrew.md`, which now carries the
stage's executable plan and its execution record. **This file goes only once the third stage has
left it**: Phase 5 checks that the new plan carries every remaining live decision before deleting
this programme. This is about this file alone. The spent WLC model plan is deleted with the
second stage, as the second-stage plan specifies.
...
| **Third stage — the codex-index trio and diffable-pointed-hebrew, total evacuation** | [PLAN-evacuate-the-codex-index-trio-and-diffable-pointed-hebrew.md](PLAN-evacuate-the-codex-index-trio-and-diffable-pointed-hebrew.md) | **Phases 0–4 DONE 2026-09-04.** Fresh clean-tree measurements established the Leningrad baseline; Phase 1 moved its five retained artifacts, repointed its generator, archived the source, and removed its workspace and visibility entries. The primary clone remains only as shared Git metadata for the manifest-pinned review-forest input. Phase 2 moved Aleppo's 170 source blobs, repointed its readers, deployed and verified the three-page redirect host, and removed its clean direct clone. Phase 3 moved Cambridge 1753's 100 selected blobs under `cam1753/`, retained only the 14 spreads, regenerated the ignored 28-page JPEG tree from them, archived the source, and removed its clean direct clone. Phase 4 moved diffable-pointed-hebrew's product data and command into MAM-basics, removed the source from workspace, visibility, and vendoring configuration, and pushed its dated breadcrumb before removing the verified primary clone. Ben decides separately whether to archive the source repository. Ben's 2026-09-04 decision retains one temporary pinned `MAM-XML/` tree for the Aleppo and Cambridge 1753 readers until the later MAM-simple evacuation replaces it. The stage reverses this file's "Decision — total evacuation for three repos, Python-only for the codex-index trio" for the trio, and Appendix B for diffable-pointed-hebrew, on the 2026-09-02 single-worktree ground. |
