...
| **Fourth stage — the five MAM products, total evacuation** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md) | **Phase 0 DONE 2026-09-05 — planning only.** The dedicated plan records all six 2026-09-05 decisions, re-measures the current product facts, and starts no product lane. The five lane order is MAM-simple, MAM-for-Sefaria, MAM-parsed, MAM-with-doc, then MAM-OSIS. The stage takes MAM-basics' product writes outside MAM-basics from five to zero; MAM-private's separate census program remains out of scope. |
