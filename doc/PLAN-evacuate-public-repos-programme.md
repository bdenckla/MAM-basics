...
Phase 3 paused before Land for a historical MAM-parsed revision decision
...
| **Fourth stage — the five MAM products, total evacuation** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md) | **Phases 1–2 DONE 2026-09-06; Phase 3 paused before Land.** MAM-simple landed in MAM-basics at `cf7c7a35`; source commit `9a350be` retained a verified Pages redirect host, and the clean source clone moved to the Windows Recycle Bin. MAM-for-Sefaria landed at `4195440e`; source commit `cf23b47` retained a verified Pages redirect host, while the clean source clone stays in place pending the explicit safety report. MAM-simple's corpus, example, vendoring, and Cambridge 1753 line-break oracles passed. MAM-for-Sefaria's 168 selected blobs, product regeneration, source-mtime check, 39-CSV example oracle, and both live Pages paths passed. Phase 3 re-measured MAM-parsed `5108203` at 96 files / 29,937,275 blob bytes / 22 legacy HTML paths and passed the baseline suite with 974 tests and 5 skips. The change-log readers require historical source commits absent from MAM-basics, and Empty would remove `plus/` from source `HEAD`; a path-only repoint fails. The dedicated plan's Phase 3 preflight records the reproduced failure, README dispositions, and a proposed separate history cache pending Ben's decision. MAM-basics was initially `bf8969e1` and advanced through an unrelated merge to `ae450bd0` before the documentation-only record; MAM-parsed remains unchanged, and no lane step or source removal occurred. The remaining lane order is MAM-parsed, MAM-with-doc, then MAM-OSIS. MAM-private's separate census program remains out of scope. |
