...
| **Second stage — the total evacuation of book-of-job, holman-ketiv-qere and UXLC-utils** | [PLAN-evacuate-the-rest-of-three-repos.md](PLAN-evacuate-the-rest-of-three-repos.md) | **DONE 2026-09-03.** All six phases are complete: each source tree landed under its pure MAM-basics prefix, its redirect host retains generated stubs, its source clone left the workspace and its downstream documentation is current. Phase 6 recorded Ben's no-entry `frozen_repos` decision, completed cross-repo bookkeeping and confirmed the then-15-folder workspace. The temporary codex-index-leningrad sparse vendor remained under Ben's 2026-09-03 authorization; the third stage removed it the next day. The plan remains because the fourth-stage plan relies on its lane and Decision F. |
...
| **Fourth stage — the five MAM products, total evacuation** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md) | **DONE 2026-09-10 — Phases 1–6 and all four stages.** MAM-simple landed at `cf7c7a35`, its redirect host at `9a350be`, and its clean clone was recycled. MAM-for-Sefaria landed at `4195440e`, its redirect host at `cf23b47`; its clone remains pending its separate safety report. MAM-parsed Land `63cf6c98` preserved 95 source blobs byte-identically; Licence `8176e91d` stored 144 historical JSON blobs verified against their original revisions. Repoint `8e5735fb` and Stubs preparation `2a50fd15` passed product, example, and historical differential checks. Common comparisons use permanent tracked inputs; arbitrary comparisons explicitly require read-only access to a sibling MAM-parsed clone. No cache, automatic clone, or fetch is used. Following Ben's direct deletion approval, source `c9e04c4` retained 27 redirect-host files and deployed successfully; all 22 legacy/target URL pairs passed live checks. Roster commit `9a61e7b5` removed MAM-parsed from the workspace and visibility map together. The clean clone was moved to the Windows Recycle Bin after its refs, objects, and worktree safety checks. With the actual clone absent, regeneration produced zero Git content differences, all historical reports matched, and the canonical suite passed 975 tests with 5 skipped in 93.67 seconds. The incidental site-index lint failure was corrected separately in `d3d7e321`; the smaller workspace also lost stale retired-product entries. All four Phase 3 oracle layers are complete. Phase 4 then landed and verified MAM-with-doc, and source `904c9fa` became its deployed redirect host. Phase 5 completed MAM-OSIS at MAM-basics `a2e883f1` and source redirect-host `8df241b3`; the clean source clone was recycled. Phase 6A commit `11edc15e` froze 2,450 selected blobs and the command map. Phase 6B commit `dc043165` regenerated parsing, FOI, surveys, documentation and MAM-simple with zero output-byte changes. Phase 6C then regenerated all 99 MAM-with-doc and historical-comparison outputs with zero output-byte changes, preserved all 144 permanent historical JSONs, and passed 57 affected tests. Phase 6D task `01a08c78-1b03-78e2-922a-9f3772e5801b` regenerated all 295 Sefaria, OSIS, canonical and independent-example outputs without changing any output bytes; all 295 filtered Git blobs match the Phase 6A baseline. The 106 independent comparisons comprise 104 exact raw-byte pairs and two intentional one-line Sefaria provenance differences. All 45 support pairs match as Git blobs, all 27 historical OSIS files remain unchanged, local XSD validation passed, and 9 affected tests passed. Permanent evidence for 6D is `in/mam_products_phase6d_verification.json`. Phase 6E task `01a08c94-01be-73d0-9d6a-29a5664a9585` then verified all 144 frozen source redirect objects and 484 live HTTP responses, regenerated vendoring and bookkeeping outputs, classified the complete source-reference sweep, and resolved the two bounded public findings in `3e3a93ef`. Pages run `34523189844` deployed the corrected MAM-with-doc sigil page byte-identically. The vendoring audit now distinguishes 19 raw-identical support copies from 25 CRLF/LF-only copies; all 44 audited copies and all 45 complete support pairs match as filtered Git blobs. The affected set passed 60 tests. Permanent evidence is `in/mam_products_phase6e_verification.json`. Phase 6F then established that every one of the 29 recorded Phase 6B–6E commands remains valid at `40b8ea4b`: no implementation, direct input or output path changed after the command's accepted commit, so no generator rerun was required. All 2,450 baseline entries and every recorded output set remain exact. The final inventory covers 5,649 tracked files / 834,533,018 Git bytes, including 1,154 files / 332,326,987 bytes across the five landed product and published subtrees. The canonical suite passed 989 tests with 5 skipped; all five source hosts and deployments remain exact, all five live source indexes match their source Git blobs, and all five live target product indexes match MAM-basics. The five actual source checkouts remain absent. Programme-completion commit `12070fc63b527a8f8de7cbfd040f2f17a146ed9a` was pushed and deployed successfully by Pages run `34527262663`; fresh checks matched all five source indexes and all five target product indexes to their Git blobs. Permanent evidence is `in/mam_products_phase6f_verification.json`. Phase 6, the fourth stage and this programme are complete; MAM-private was not modified, the full mega was not run, and no successor was created. |
| **Correction, 2026-09-09 — Phase 2 clone retirement** | September 8 review, finding 5.2 | The preceding row's MAM-for-Sefaria clone-retirement status is superseded by `9cf48863` (2026-09-08), which records the completed safety report, Ben's approval, and the move of `C:/Users/BenDe/GitRepos/MAM-for-Sefaria` to the Windows Recycle Bin in the five-products plan and September 7 remediation plan. This cites the recorded retirement; no fresh filesystem safety audit or cleanup was performed. |
| **Fourth stage — Phase 4 MAM-with-doc completion** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md) | **DONE 2026-09-06.** Land commit `5a28bc0e` copied 272 selected blobs from source `0fe406c` with zero blob mismatches; source Pages workflow stayed at the redirect host. Target commit `3c8c9750` landed local writers and readers, licensing, the 113-path manifest `in/mam_with_doc_redirect_pages.json`, the workspace/visibility removal, and generated output. Regeneration changed zero source files and a second pass changed zero target bytes. MAM-parsed-plus reports 180 raw changes, 56 records, and 58 published cards. Source `904c9fa` now retains a dated README, Pages workflow, 113 stubs, and `404.html`; both Pages runs (`34061866037` and `34062108606`) succeeded, and the target, legacy stub, and custom 404 passed HTTPS checks. After clean-head, worktree, ref, and object safety checks, `C:/Users/BenDe/GitRepos/MAM-with-doc` moved to the Windows Recycle Bin. MAM-OSIS remains the only later product lane; MAM-private was not modified. |
| **Correction, 2026-09-09 — Phase 4 commit attribution** | September 8 review, finding 5.2 | In the preceding row, the workspace and visibility-map removal belongs to `19df42f3`, which removed MAM-with-doc from both workspace files and `repo_visibility` in `in/repo_maintenance_policy.json`. The attribution of that removal to `3c8c9750` is superseded. The five-products plan already records the corrected attribution. |
| **Fourth stage — MAM-with-doc Phase 4 canonical verification** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md) | **DONE 2026-09-06.** The first canonical run found one stale generated MAM-simple versification page that retained a retired MAM-with-doc URL. `py/main_mam_simple.py doc-only` changed only that page; the targeted canonical test then passed 7 tests and 27 subtests. The final canonical suite passed 976 tests with 5 skipped and 65 subtests in 90.70 seconds. |
| **Fourth stage — Phase 5 MAM-OSIS preflight and task boundaries** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md) | **5A DONE 2026-09-10; migration remains open.** Ben requested bite-sized chained tasks to minimize context compaction. The dedicated plan now separates preflight, Land/Licence, Repoint, Stubs, Empty, and Remove/completion as tasks 5A–5F. Verified clean primary heads were MAM-basics `9d1de07404bd9257c3e4cafc0acc1c46b87264c9` and MAM-OSIS `697dc98a904a52ed81bb105772f002d3efb360e3`, both matching live remote `main`. Git-blob inventories: MAM-basics 5,547 files / 798,180,091 bytes; MAM-OSIS 92 / 29,822,402, including 30 published files and one HTML page. All 24 source OSIS book blobs match the committed MAM-simple example outputs. Canonical baseline: 988 passed, 5 skipped in 99.30 seconds. The historical source snapshot's CC BY-SA 3.0 terms need separate scoping from the current product's CC BY-SA 4.0; the source also tracks an obsolete scratch commit message and a hygiene checker whose scope must merge into the canonical checker. No product generator, Land, redirect flip, Empty, or Remove ran in 5A. Task 5B performs only Land and Licence before creating 5C. MAM-private and Phase 6 remain out of scope. |
| **Fourth stage — Phase 5 MAM-OSIS Land and Licence** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-5b-land-and-licence-record--2026-09-10) | **5B DONE 2026-09-10; 5C–5F remain.** Land `1c817b5307ca535a5fc04ede8cd98add4a79a6f6` preserves 90 files / 29,821,603 bytes from source `697dc98a904a52ed81bb105772f002d3efb360e3`, with every source path classified in `in/mam_osis_land_manifest.json`. The 686-byte workflow stays at the source host; the obsolete 113-byte scratch commit message is excluded. Every selected staged blob, mode, size, and SHA-256 matched; all 24 OSIS book blobs equal the committed MAM-simple examples. The complete README, historical snapshots, and static assets remain. The manifest records inherited end-of-file whitespace in 21 historical XML files; all remaining Land paths pass the whitespace check. Licence `c39b6dd69e91eacd7f2ab878fe771db89972e214` separately records current CC BY-SA 4.0 and the historical MAPM snapshot's CC BY-SA 3.0 Unported notice. Canonical verification: 988 passed, 5 skipped in 129.93 seconds; no checker edit was required. The copied checker remains an unchanged source blob awaiting scope merge and removal in 5C, which must also update its licence-inventory description. Target Pages run `34489367085` succeeded; all 30 deployed files / 782,189 bytes returned HTTP 200 and matched source SHA-256, including every page asset. No source commit, product regeneration, redirect flip, roster change, Empty, or Remove occurred. Next is task 5C only; MAM-private and Phase 6 remain out of scope. |
| **Fourth stage — Phase 5 MAM-OSIS Repoint and product verification** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-5c-repoint-and-product-verification-record--2026-09-10) | **5C DONE 2026-09-10; 5D–5F remain.** Repoint `ef570b8c745c40e28f8594bdceac6bdea5d3f818` moves production book/header/combined/page paths into MAM-basics, adapts README and product URLs, and updates pipeline labels and current path documentation. The canonical hygiene checker gains the proper product scope; the copied checker is removed and its licence-inventory description updated. All 27 production files and 24 independent example files regenerate byte-identically to their frozen oracles; every source mtime is unchanged. The permanent `in/mam_osis_repoint_verification.json` records exact paths, mtimes, hashes, and file dispositions without changing the Land manifest. All 44 example support files match their canonical sources. Graph regeneration found and fixed the DOT writer's CRLF output; the final graph changes only the OSIS label and resulting geometry. Canonical suite: 988 passed, 5 skipped in 91.35 seconds; affected lints: 24 passed in 17.02 seconds; Black, Ruff, and ordinary whitespace checks passed. Source remains unchanged at `697dc98a904a52ed81bb105772f002d3efb360e3`; all 30 target published blobs still match 5B. No redirects, roster changes, Empty, Remove, full mega, or MAM-private changes occurred. Next is task 5D only; Phase 6 remains out of scope. |
| **Fourth stage — Phase 5 MAM-OSIS Stubs and published URLs** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-5d-stubs-and-published-urls-record--2026-09-10) | **5D DONE 2026-09-10; 5E–5F remain.** Target preparation `d61c3472981f927d69a5a7452690d5130078a1c5` freezes the unchanged source’s sole legacy HTML path in `in/mam_osis_redirect_pages.json`, registers the redirect host and sibling declaration, and adds the authored site-index entry. The existing table-driven manifest lint automatically gains its MAM-OSIS case. The actual site-index generator changed only one line in `gh-pages/index.html`; no source mtime changed. Target Pages run `34493549562` succeeded before the source flip, and all 30 target files / 782,189 bytes returned HTTP 200 with exact frozen-source hashes. Source `26a7e85f8cb211043c33ef2badfddb4ac69d5ee9` changes only Pages: index modified in place, custom 404 added, and 29 obsolete assets removed. Source run `34493924015` succeeded; all 33 HTTP checks passed. Live browser checks proved query/fragment preservation, custom-404 path forwarding, and the three percent-encoded PNG redirects; all 28 target images loaded. The report records the target’s lack of authored anchors and an optional CSS browser-navigation timeout separately from the successful HTTP checks. Canonical suite: 989 passed, 5 skipped in 93.90 seconds; affected lints: 15 passed in 3.45 seconds; Black, Ruff, and whitespace checks passed. Permanent evidence is `in/mam_osis_stubs_verification.json`. All 62 non-published source files and both workspace rosters plus `repo_visibility` remain unchanged for 5E. The source clone remains for 5F; no MAM-private modification or full mega occurred. Next is 5E only; do not begin or create Phase 6. |
| **Fourth stage — Phase 5 MAM-OSIS Empty** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-5e-empty-record--2026-09-10) | **5E DONE 2026-09-10; 5F remains.** Source `8df241b3bb43e6f8ecf76ee35175380689fbcb68` removes 58 remaining source files and leaves six redirect-host blobs / 3,475 bytes; the workflow and generated index/custom 404 are unchanged. The dated README links to the landed product and Pages target, preserves the history and externally cited URL rationale, and accurately records an enabled source issue tracker with no issues. The landed substantive README is unchanged. Target `1510fc2d1fef6d2520c4a8b0a31a56618e8f5615` removes only MAM-OSIS from both workspaces and repo_visibility: folder counts 6 to 5 and 2 to 1; visibility repository entries 6 to 5 (map keys including the comment, 7 to 6). The redirect-only sibling declaration remains; no freeze or keep-absent entry was added. All prior permanent evidence, product and historical files, and example source/output trees are unchanged. Fresh oracles: exactly 27 production and 24 independent example writes, byte-identical to frozen blobs, with zero source mtime changes. Canonical suite before Empty: 989 passed, 5 skipped in 103.23 seconds; affected lints after Empty: 33 passed in 16.05 seconds. Source run `34496487629` and target run `34496496618` succeeded; 33 legacy responses and all 30 target files / 782,189 bytes passed exact HTTP/hash checks. The unchanged deployed scripts retain the documented 5D browser proof. Permanent evidence is `in/mam_osis_empty_verification.json`. Before editing, a concurrent integration advanced the target from `9c12f43f` to `5c0016b0`; ancestry and the sole instruction-file diff were verified, with both trees clean and remote-aligned. The actual source clone remains for 5F; no recycling, MAM-private modification or full mega occurred. Create only 5F after committed/pushed write-back and final clean-head checks; do not begin or create Phase 6. |
| **Fourth stage — Phase 5 MAM-OSIS Remove and completion** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-5f-remove-and-phase-5-completion-record--2026-09-10) | **Phase 5 DONE 2026-09-10; Phase 6 has not begun.** Task 5F verified clean primary MAM-basics `a04a4cd9e3051b7152e50905be357627349e36d7` and source `8df241b3bb43e6f8ecf76ee35175380689fbcb68`, each matching tracking and live remote main. Source main and all five tags match live refs; all 1,620 Git objects are reachable from live remote tips. No unpushed commits, stashes, unreachable objects, ignored/untracked files, alternate object store, reparse points or linked worktrees remain. Only `C:/Users/BenDe/GitRepos/MAM-OSIS` was recycled, with the matching Windows Recycle Bin item and actual sibling absence verified. With that actual source absent, production wrote exactly 27 destination files and the independent example exactly 24, all byte-identical to frozen Land and committed example blobs. Canonical suite: 989 passed, 5 skipped in 98.10 seconds; affected roster/redirect/site-index/sibling/hygiene checks: 33 passed in 16.93 seconds. Scratch redirect build and explicit-directory check passed without recreating the source. All 90 selected Land blobs, 24 committed examples, 44 support files and 172 protected destination paths reverified. Source run `34496487629` and target run `34497273777` succeeded; fresh checks verified 33 source responses and all 30 target files / 782,189 bytes against generated stubs and frozen hashes. The unchanged deployed bytes preserve the six documented 5D browser cases and their stated limits. Permanent evidence is `in/mam_osis_remove_verification.json`; `CLAUDE.md` records the cross-machine roster decision and explicit temporary-clone stub-publication procedure. All four oracle layers are complete. The source remains an unarchived deployed host; roster and redirect-only sibling declarations are unchanged from 5E. No MAM-private modification, full mega, or Phase 6 work occurred, and no successor was created. |
| **Fourth stage — Phase 6 authorization and task boundaries** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-6-session-boundaries--bens-decision-2026-09-10) | **6A–6F DONE 2026-09-10; Phase 6 and the programme are complete.** After Phase 5 completion at `a2e883f1227e11691ce51f95ba77a9ac4192e55a`, Ben requested fresh sessions sized to avoid context compaction. Phase 6 was divided into sequential tasks 6A–6F: preflight/command map/baseline; MAM-parsed and MAM-simple; MAM-with-doc and historical comparisons; Sefaria/OSIS and the independent examples; publication/vendoring/source references; final checks/inventories/programme completion. Each task committed, pushed and verified a clean primary checkout before its successor; 6F created no successor. Use the saved GitRepos project directly in Local mode. Ben selected `gpt-5.6-sol` with `xhigh` reasoning for the remaining Phase 6 chain on 2026-09-10. The full mega, MAM-private modifications and unrelated editorial work remained outside scope. The earlier 5A–5F stop instruction is superseded by this authorization; dated completion records remain unchanged. |
| **Fourth stage — Phase 6A preflight, command map and baseline** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-6a-preflight-command-map-and-baseline-record--2026-09-10); [command map](mam-products-phase6-command-map.md) | **6A DONE 2026-09-10; 6B–6F remain.** Verified clean primary `main` at `9ea4e39abf6ceae3e52f0cb37a3ba4abf470b05f`, equal to tracking and live remote. All five actual source siblings are absent; the five unarchived source hosts have exact breadcrumb/stub inventories, successful current-head deployments and byte-verified live indexes. `in/mam_products_phase6_baseline.json` freezes 2,450 target blobs / 351,416,216 bytes, exact command/input/output sets, original source trees and distinct Land/current identities. Whole target tree: 5,642 files / 828,491,627 bytes. All 144 historical parsed JSONs / 84,572,003 bytes match their permanent manifest; all 92 original OSIS entries match its unchanged manifest. The 106 independent pairs comprise 104 exact matches and two intentional Sefaria provenance differences; all 45 support module copies match. Canonical suite: 989 passed, 5 skipped in 117.70 seconds; no subtest total printed. `in/mam_products_phase6a_verification.json` retains the evidence and bounded findings. The map includes template surveys/diagrams, repeated parsed-doc verification after surveys, authored MAM-with-doc pages, all four vendoring reports and graph/site bookkeeping. No product generator, download refresh, clone recreation, recycling or MAM-private write occurred. Current product synchronization and complete URL coverage remain the assigned 6B–6E work; 6F performs final validity checks and programme completion. Create only 6B after committed/pushed clean write-back. |
| **Fourth stage — Phase 6B parsing, surveys and MAM-simple** | [PLAN-evacuate-five-MAM-products.md](PLAN-evacuate-five-MAM-products.md#phase-6b-parsing-surveys-and-mam-simple-record--2026-09-10) | **6B DONE 2026-09-10; 6C–6F remain, with a model-choice pause before 6C.** Task `01a08c3c-557c-73c0-92aa-bf632c4a40d2` verified clean primary `main`, tracking and live remote at `11edc15e73cc39f1b6cc2e8c5778ea6a06a399ca`. Parse, FOI, template surveys, repeated parsed-documentation verification and the full MAM-simple CLI all passed: 415 distinct outputs / 116,606,861 bytes match the frozen baseline exactly. Primary mtime counts were 74, 49, 26, 24 and 261; five conditional MAM-simple documentation/assets files were already current. Both documentation passes reported 79 passed, zero failed and the same named pending check. Graphviz matched the pin; grammar locks and all five actual source absences were preserved. The physical-root snapshots also observed concurrent edits in a separate nested Claude worktree; primary inputs and files outside each command's output set were unchanged. All 144 historical parsed JSONs / 84,572,003 bytes, 188 protected static paths and 11 prior JSON evidence files remain intact. All 45 support copies match source Git blobs, with 25 existing source CRLF versus copied LF differences recorded explicitly. Affected checks: 74 passed, 27 subtests passed in 7.95 seconds. Staged Unicode/prose hygiene: 7 passed in 13.08 seconds; record validation and whitespace checks passed. Permanent evidence: `in/mam_products_phase6b_verification.json`. No production source or generated bytes changed. Ben requested completion and push of 6B followed by a pause before creating 6C so he can choose its model; no successor has been created. |
...
1. **Redirect stubs for codex-index-aleppo, and an archive for codex-index-leningrad and
   codex-index-cam1753** — decision 2 above.
...
## Fourth stage — the five MAM products, total evacuation — **DONE 2026-09-10**
...
Phases 1–4 as complete through 2026-09-06 and Phase 5, MAM-OSIS, as complete
on 2026-09-10. Ben separately authorized Phase 6 on 2026-09-10 as the fresh
sequential 6A–6F task chain recorded in the dedicated plan. Tasks 6A–6F completed
on 2026-09-10, completing Phase 6, the fourth stage and this programme. Ben
selected `gpt-5.6-sol` with `xhigh` reasoning for the final task. Phase 6 was
not part of the earlier 5A–5F authorization.
...
2. **MAM-simple stops being a small clone an outsider can run. UNRESOLVED, and it is the objection
   ...
   **Correction, 2026-09-09 (September 8 review, finding 5.2):** the historical
   "neither exists today" statement is superseded for sparse-checkout instructions.
   The landed [MAM-simple README](../MAM-simple/README.md#sparse-checkout) and
   [MAM-parsed README](../MAM-parsed/README.md#download-only-this-product) now give
   `git clone --filter=blob:none --sparse` and product-selection commands. MAM-parsed's
   instructions also state that no release archive is maintained. This note records
   the later documentation; the original objection remains above.
...
`PLAN-evacuate-the-rest-of-wlc-utils.md`'s Phase 6 (the plan was deleted 2026-09-03
by `80c9ad85` and remains in git history)
