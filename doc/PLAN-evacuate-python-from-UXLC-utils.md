...
| 6 — flip the provenance breadcrumbs and disambiguate issue citations | **done** 2026-08-18, commit `30cdfd2` here and `9be1431` in UXLC-utils. Breadcrumb: one sentence, one artifact. Citations: 57 sites read, **50 prefixed `UXLC-utils#`, 7 left bare** (two design-doc §9 questions, three UXLC change numbers, MAM-basics' own #218/#219). `xml_source` re-checked and correctly left alone. `CLAUDE.md` is now "Three issue trackers" |
| 7 — cross-repo bookkeeping | **done** 2026-08-18. **Item 1** inside Phase 4 (2026-08-03, `2b5c87c` here — see §Phase 7 item 1); **item 6** 2026-08-17, github-misc `549224e`; **items 2–5** 2026-08-18, no commit owed — 2 and 3 confirmed as predicted, 4 deleted a 33 MB orphaned venv holding 832 `.py` and found no orphaned worktrees, 5's two greps came back clean and turned up a second inventory-invisible vendoring relation in holman-ketiv-qere that is already pointed at MAM-basics. This row said a flat "not started" until the 2026-08-03 review (MAM-basics [#219](https://github.com/bdenckla/MAM-basics/issues/219), major 4) reconciled it with the Phase 4 row and §Phase 7 |
...
**Landed as `30cdfd2` here (11 files, +87/−56) and `9be1431` in UXLC-utils (one artifact).**
Every baseline in §Status was re-measured first and every one matched: MAM-basics at `a70ee8d`,
clean, **1,910 tracked files**, suite **905 passed / 5 skipped**, `ruff check py` clean; UXLC-utils
at `f99610a`, clean, **780 tracked files and 0 tracked `.py`**, one worktree. No other session was
live in either repo.

**Part 1 — the breadcrumb. One sentence, one artifact, exactly as Phase 1 predicted.**
`py/uxlc_fois/fois_html.py`'s `_write_index` now writes "generated by
**MAM-basics/**py/main_fois.py" — the repo name *added* rather than one name swapped for another,
in the `<repo>/py/...` shape `mb_cmn/provenance.py`'s `generated_by_text` already uses. The string
is wrapped across two source lines to stay inside black's 88; the rendered sentence is otherwise
unchanged. Its artifact, `gh-pages/fois/index.html`, lives in **UXLC-utils**, so the generator and
its output were committed in different repos.

**Part 2 — the `xml_source` field. Re-checked, and this section's reasoning holds unchanged.**
`in/UXLC-misc/2026.04.01-map-to-book-of-job.json` is still a **UXLC-utils** file: `git ls-files
in/UXLC-misc` returns 31 there and 2 here. **The `in/UXLC-misc/` that now exists in MAM-basics is a
different tree wearing the same name** — its `_provenance.md` records `all_changes.json` as copied
from UXLC-utils' `out/UXLC-misc` on 2026-08-12, and it holds nothing else. So the field's literal
`in/UXLC-misc/2026.04.01 - Changes.xml` is still read relative to the repo holding both the JSON
and the XML it names, and it was left alone. **A confirmation, not a finding** — but the name
collision is recorded here so the next session does not re-derive it.

**Part 3 — the issue citations. 57 sites read, 50 prefixed, 7 deliberately left bare.** The count
matched this section's measurement exactly. Every distinct number was checked against
`bdenckla/UXLC-utils` with `gh issue view` before prefixing, and all eighteen that took a prefix
came back CLC issues whose titles match their citation's sentence. **Seven did not take one, and
three of those seven have a real UXLC-utils issue of that number to be mistaken for** — which is
why the number alone never settles it:

- **`clc_collect.py:47`'s "design doc §9 #2" and `clc_render.py`'s `#6` name `clc-design.md`'s own
  numbered open-questions list**, not issues. `## 9. Open questions (consolidated)` item 6 *is* the
  always-link reversal `clc_render.py`'s comment describes, whereas UXLC-utils#6 is "Review
  WLC-only utilities that may not belong in UXLC-utils". **That site read "issue #6" and was
  already wrong before this phase touched it** — inherited from UXLC-utils `462790e` (2026-07-04),
  whose commit subject calls the design doc's numbered question an issue. It now reads "design doc
  §9 #6, not an issue", which is what the neighbouring `clc_collect.py` site had said all along.
  **Leaving it bare would have been wrong too**: a bare `#6` here resolves to MAM-basics #6, "Try
  to merge functionality of main_osis*.py".
- **Three sites name a UXLC *change* number**, the 2026.10.19 release's tenth change:
  `clc_collect.py:73` "change #10", `clc_collect.py:87` and `clc_dual_cant.py:879` "pending change
  #10". UXLC-utils#10 exists and is about `out/uxlc-words.json`.
- **`main_uxlc_grammar_test.py`'s `#218` and `#219` are MAM-basics' own** and were already right
  bare; UXLC-utils' numbering stops at 56. Its two `wlc-utils#9` citations were already prefixed
  and were not touched — the prefixing refuses a `#` preceded by a word character or hyphen. **So
  that file is the one of the ten with no diff at all.**

**Line lengths: only `clc_versification.py` was re-wrapped.** A 12-character prefix on a
hand-wrapped comment pushes lines over, and `ruff.toml` deliberately does not enforce E501, so the
test used was what each file already tolerated in *display* columns — combining marks excluded,
several of these lines carrying pointed Hebrew where codepoint count is not width.
`clc_dual_cant.py` and `clc_render.py` already ran to 101 and now reach 113, near enough that they
were left, which keeps their diff purely mechanical and so trivially reviewable.
`clc_versification.py` ran to 93 and one line reached 119, plainly out of that file's character;
its two affected docstring paragraphs were re-wrapped with the word sequence preserved.

**`CLAUDE.md` grew its UXLC-utils clause, and the section is now "Three issue trackers".**
UXLC-utils keeps its 56 issues, numbered 1–56, so the **whole** numbered range collides with
MAM-basics — sharper than wlc-utils', whose clause could name a 1-88 range. Three worked collisions
are given, the CLC non-issue traps above are folded into the existing "Not every `#NN` is an issue"
bullet, and a closing paragraph records that UXLC-utils' own `doc/` was left alone on the same
reasoning wlc-utils' was. **The rename strands four pointers**: four sentences in
`doc/PLAN-evacuate-the-rest-of-wlc-utils.md` cite the section as "Two issue trackers". They are
that plan's execution record, describing the section as it stood when each phase ran, so they were
**left as written** and the `CLAUDE.md` section says so — the answer Ben chose for the stale
`../masorah-books/` paths on 2026-08-10, one sentence in place of edits to the citing sites.

**Verification.** The full oracle ran from `C:\Users\BenDe\GitRepos\MAM-basics` as the working
directory, main checkout: `main_uxlc_mega.py`, then `main_clc.py`, then
`main_map_changes_to_book_of_job.py`. **UXLC-utils came back with exactly one modified file,
`gh-pages/fois/index.html`, differing by exactly the one breadcrumb sentence**; the other 213
tracked artifacts byte-identical, and `git status --porcelain -- in out gh-pages data` empty here,
the citation edits being comments and docstrings only. Suite **905 passed / 5 skipped**, unchanged;
`ruff check py` clean; black left all 14 touched files unchanged. **The whole oracle was run a
second time after Phase 7 deleted UXLC-utils' `.venv`, with the same one-file result.**

**One thing worth carrying to the next repo: `--help` is not a flag on `main_uxlc_mega.py`.** It
parses no arguments, so `py/main_uxlc_mega.py --help` silently runs the entire five-step pipeline
instead of printing usage. Harmless here — the run was wanted anyway — but a session expecting
usage text will regenerate a corpus without meaning to. Read the source, not `--help`.
...
## Phase 7 — cross-repo bookkeeping — DONE 2026-08-18 (items 2–5; 1 and 6 were already done)

**Items 2, 3 and 5 were run rather than assumed, and all three came back as this section
predicted. Item 4 found and deleted a 33 MB orphaned venv and no orphaned worktrees.** Nothing
here needed a commit in MAM-basics: every item was a confirmation, a grep, or a deletion of an
untracked directory.

**Item 2 — confirmed, no change.** `all-repos.code-workspace` lists `../UXLC-utils` (line 58) and
it stays: that repo still tracks 780 files. `in/repo_maintenance_policy.json`'s `frozen_repos`
does not name UXLC-utils and did not need to.

**Item 3 — confirmed, and the predicted string came back verbatim.**
`--run-black --workspace-file all-repos.code-workspace --repos UXLC-utils` reports
`REPO=UXLC-utils; BLACK_ATTEMPTED=False; BLACK_OK=False; Skipped: no tracked .py files in this
repo`. `--check-repo-standards` on the same repo degrades just as gracefully:
`MAINTENANCE_SCRIPT=n/a; WORKTREE_STEP=n/a; PATH_UTILITY=n/a`, with `LINKED_WORKTREES=0`,
`AGENT_BRANCHES=0`, `SYS_PATH_MUTATIONS=0`, `GITATTRIBUTES_LF=True`. Both runs were scoped with
`--repos`, since dropping it **reformats every repo in the workspace**.

**Item 4 — one orphaned venv, 33 MB, 832 `.py` inside it; no orphaned worktrees.** The shape
wlc-utils' Phase 7 warned of, at 832 files against its 789. Done in the order this plan requires:

- **`--clean-worktrees` ran first**, before anything else touched a worktree, and reported
  `worktrees: nothing to clean`. Hand-running `git status` inside a worktree makes that sweep skip
  it, so the sweep goes before the poking, not after.
- **Then the checks the deletion was conditional on.** Every `.venv` mention left in UXLC-utils
  names **MAM-basics'** venv by absolute path — `CLAUDE.md:17`, `README.md:31` and `:37`, and eight
  lines of `.novc/robots-retest-2026-08-12.md`, all spelling
  `C:\Users\BenDe\GitRepos\MAM-basics\.venv\Scripts\python.exe`. The only other hit,
  `.novc/baseline_checker.py:17`, is the string `"/.venv/"` in a path filter, not an invocation.
  **UXLC-utils' own `CLAUDE.md` says it outright**: "whatever `.venv` is left here has nothing to
  run." Zero tracked files under `.venv`, which is self-ignoring via the `.venv/.gitignore` that
  `python -m venv` writes.
- **And the junction check this plan's sibling rules demand**, because a junction here would have
  taken MAM-basics' venv with it: `Get-Item -Force` reported `Attributes: Directory`, no
  `ReparsePoint`, empty `LinkType` and `Target`, and its own `pyvenv.cfg` naming
  `C:\Users\BenDe\GitRepos\UXLC-utils\.venv` as what `python -m venv` was pointed at. A real
  directory, so `Remove-Item -Recurse -Force` was safe; MAM-basics'
  `.venv\Scripts\python.exe` was confirmed present afterwards, and the full oracle re-run clean.

**Item 5 — both greps run, and both came back clean.**

- **`UXLC-utils/py` across all nineteen sibling repos: zero real hits.** The only matches anywhere
  are three lines of *this file* describing the grep. codex-index-leningrad, the known consumer, is
  clean — Phase 5's drop held.
- **`book-of-job`: the dependency runs the way Phase 1 said, and it is declared.**
  `main_map_changes_to_book_of_job.py` reaches that sibling through
  `uxlc_paths.require_book_of_job_dir()`, which is `paths.require_sibling("book-of-job", ...)`, so
  a missing clone fails naming its overrides rather than composing a path into nothing.
- **The grep did turn up a second vendoring relation the inventory cannot see, and it is already
  pointed at the right repo.** `holman-ketiv-qere/py/uxlc_paths.py` is **byte-identical** to this
  repo's `py/uxlc_paths.py`, is actively imported there by `uxlc_misc/my_uxlc.py` and
  `uxlc_misc/my_uxlc_page_break_info.py`, and is **absent from `doc/vendoring-inventory.md`** —
  whose scan looks for `mb_*` packages, and this is a loose file at the top of `py/`. That is the
  programme's cross-cutting finding 2 recurring exactly as it predicted ("evidence that a second
  such relation may exist somewhere the scan cannot see"). **No action was owed**: that repo's
  `main_update_vendored_files.py` names `py/uxlc_paths.py` in `_VENDORED_FILES` and syncs it from
  `../MAM-basics`, and its `py/_provenance.md` records the copy as taken from `MAM-basics/py` on
  2026-08-12 — post-evacuation, so it never pointed at UXLC-utils. **Hand it to
  `PLAN-evacuate-python-from-holman-ketiv-qere.md`**, which is next in the Order: that plan must
  know its repo vendors a loose `uxlc_paths.py` the inventory does not list.
