...
untracked `.py` files, after `--clean-worktrees` found nothing to clean.** **Landed as `0cce58f`
here — this record and the three Status entries it moves — and nothing at all in book-of-job:**
every item was a confirmation, a grep, or the deletion of an untracked
...
**ALL FIVE CITATIONS WERE FIXED LATER THE SAME DAY, 2026-08-22, on Ben's say-so** — see "The
disposition of item 6" below. The five are described here as this phase found and left them,
because what each said, and why each was left rather than fixed, is what the record is for.
...
other repos that still hold it.** Phases
1, 3, 4 and 6 each put it to Ben and none picked; this phase did not pick either.

**This section named those repos when it was written, and named them wrong — see the correction
below, added the same day.**

### CORRECTION, 2026-08-22: the question named three repos that hold no `provenance.py`, and the change it proposes would regress a live artifact

Ben asked, after this phase reported, when the three things it left him would become blocking.
Answering that meant measuring the third, and the measurement falsifies the premise. **The
sentence above said "the two codex-index repos and diffable-pointed-hebrew" until this correction
landed, and Phase 6's record still says it at the section "The open question is still open, and
this phase did not touch it"** — left as written, per the same convention by which Phase 1's
superseded `59 subtests` reading is left in place and pointed at rather than edited.

**Where the wrong naming came from.** Phase 1's record above, under "`mb_cmn/provenance.py`'s
`parents[2]` — the facts, and the question that is Ben's", has it right: four copies as of
2026-08-19 — MAM-basics `py/mb_cmn/`, MAM-private `al-hatorah/py/mb_cmn/`, MAM-simple
`py-examples/mb_cmn/` and book-of-job `mb_cmn/` — `parents[2]` right in three and wrong only in
book-of-job. Phase 4 deleted book-of-job's and wrote "the *other* repos that hold it" without
naming them. **Phase 6 named them, and named them wrong**, the list matching the programme plan's
line 98 — a correct sentence about which repos have a `DIFFERS` vendored copy against a copy
script, which has nothing to do with `provenance.py`. Phase 7 copied Phase 6.

**Swept 2026-08-22 across all 26 clones under `GitRepos` and `FrozenRepos`, plus MAM-private's
subtrees. Exactly three copies of `provenance.py` exist anywhere, and all three are
byte-identical** (md5 `e53232a9782827e9af80669a31452f16`), so there is no drift to re-vendor away:

| Copy | `_repo_root()`'s `parents[2]` resolves to | Right? |
|---|---|---|
...
**None of the three repos the question named holds the file.** codex-index-aleppo tracks four
`mb_cmn` files, at `py/mb_cmn/`; codex-index-cam1753 three, at a root-level `mb_cmn/`;
diffable-pointed-hebrew eight, at a root-level `mb_cmn/`; codex-index-leningrad tracks no
`mb_cmn/` at all. `provenance.py` is in none of them, so **none can reach the defect**, whatever
the depth of its `mb_cmn/`. **diffable-pointed-hebrew's `mb_cmn/file_io.py` predates the feature
outright** — it carries no `from mb_cmn import provenance`, where this repo's has one at line 8
and calls `provenance.with_json_provenance` at line 31.

**And the fact that answers the question rather than merely deflating it: the proposed change
would regress the one live consumer outside this repo.** al-hatorah emits breadcrumbs today —
`MAM-private/al-hatorah/out/a2d-override-diff-viewer/data.json` and its two neighbours carry
`"provenance": "This file was generated by
al-hatorah/py/main_3d_make_override_diff_viewer.py."`. **al-hatorah is a subtree of MAM-private,
not a repo of its own**, having moved there 2026-08-10, so a `_repo_root()` that walked to `.git`
would land on MAM-private's root and rewrite those tracked artifacts to name the wrong tree.
`parents[2]` is what makes them right. That is **Phase 3's own lesson — a `.git` walk finds a repo
root and cannot find a subtree — arriving at the very file the question is about**, and it turns
the proposed fix from harmless-but-unnecessary into a regression. MAM-simple emits too, two
`_provenance.md` sidecars, and is its own repo, so it is unaffected either way.

**So the recommendation to Ben is to close the question as a no**, on the evidence above, rather
than let a sixth phase re-put it. **HE TOOK IT, later the same day: `_repo_root()` stays
`parents[2]`, and the question is CLOSED.** The one home for that decision is
[`PLAN-evacuate-python-programme.md`](PLAN-evacuate-python-programme.md)'s section "Decision —
`mb_cmn/provenance.py`'s `_repo_root()` stays `parents[2]`", which carries the sweep, the
al-hatorah cost, and why no guardrail comment was added to the file. **No phase of any plan should
re-put it**; the sentences in this file's Phases 1, 3, 4, 6 and 7 that call it open are those
phases' records of the state before Ben settled it, and are left as written.
**The transferable rule is the one this phase broke: a list of repos is a measurement, so
re-measure it rather than copying it forward** — Phase 6 wrote a list it had not swept for, and
Phase 7 carried it into a record and into the trio's task prompt before anyone checked it against
Phase 1, four screens up in this same file.

**One of the three things left for Ben is already discharged, by the trio's Phase 0 the same day.**
The `alef_bet_to_ascii.py` citation is fixed in both public copies at once — `ef5525d` here and
`a171dd4` in codex-index-aleppo — and the two remain one blob. What that leaves of item 6's
findings is the three prose citations in MAM-private and the one in UXLC-utils' `doc/`, all four
still Ben's.

### The disposition of item 6 — all five citations fixed, 2026-08-22, in three repos and two hands

Item 6 left five stale citations reported rather than fixed, four of them because a commit to a
third repo stops and asks. **Ben was asked, said fix them, and all five are now fixed.** They went
in three commits across three repos, two of which are neither of this plan's:

| Citation | Fixed as |
|---|---|
| `py/py_ac_word_image_helper/alef_bet_to_ascii.py:5` here **and** codex-index-aleppo's byte-identical copy | `ef5525d` here and `a171dd4` there — **the trio's Phase 0**, both copies at once, still one blob |
| `mgketer/documentation/periodic-maintenance.md` §7 | `55cdc70` in MAM-private |
| `masorah-books/doc/migration-checklist.md:152` | `55cdc70` in MAM-private |
| `mgketer/py/py_ac_word_image_helper/alef_bet_to_ascii.py:6` | `55cdc70` in MAM-private |
| `UXLC-utils/doc/clc-design.md:824` | `c8db329` in UXLC-utils |

**The one that actually misled is repaired as a checklist, not merely as a path.**
`periodic-maintenance.md` §7 is a porting to-do ending "Start with `check_escape_sequences.py`",
and it told its reader to go and read a `check_all.py` harness and five `check_*.py` in
book-of-job. Its heading, its lead paragraph and a path prefix for its table now name
`MAM-basics/py/`, with the move dated; the table's five filenames are unchanged, being still the
filenames, and all six paths were verified to resolve.

**Two shapes were used, and the neighbourhood picked each** — holman-ketiv-qere's Phase 6 lesson,
applied rather than restated. `migration-checklist.md`'s entry takes `../MAM-basics/py/…`, matching
the four entries above it in the same "Do not touch" list, where MAM-private's earlier
`e8fd4ae` had used a bare `MAM-basics/py/hkq_cmn/…` for `mpu-parsing.md` because *that* file's
neighbouring bullet did. `clc-design.md` takes the bare shape too, its own §9 having used
`MAM-basics/doc/sigil-decoding.md` two lines above.

**Three things were deliberately not changed.** `migration-checklist.md`'s two other book-of-job
entries, `gh-pages/jobn-details/1413.html` and `out/enriched-quirkrecs.json`, are correct and were
verified present; its item's point — that `qr_1413.py` carries a published MAM-with-doc link that
must not be renamed — is untouched, and the old path is kept in a parenthetical so the entry stays
findable from either spelling. `clc-design.md`'s lead is untouched: book-of-job is still a small,
realistic, short-term harvest set.

**mgketer's `alef_bet_to_ascii.py` gained a paragraph the other two copies must not have.** It is
**not** the blob this repo and codex-index-aleppo share — it carries `\uXXXX` escapes where those
two carry literal Hebrew — so its docstring now says so, against the note the reconciled pair
carries telling a reader to edit those two together. It is in no vendoring policy, so nothing
syncs it either way. It parses, its docstring renders the escape example literally, and black
leaves it unchanged. `periodic-maintenance.md` has no generated counterpart, unlike the
`mpu-parsing.md` of holman-ketiv-qere's Phase 7 finding 2, so nothing needed regenerating.

**So of the three things this phase left Ben, two are closed and the third has a recommendation.**
The citations are fixed; `mb_cmn/provenance.py`'s `parents[2]` stands open only in the sense that
closing it is his, the correction above having turned it from a free choice into one with a
measured cost on the `.git` side.
