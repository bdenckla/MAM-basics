...
- **Sources that settled steps so far**, and how to re-fetch them (no local mirror exists):
  ...
  **Chapter 2's raw wikitext names the comparison edition 45 times** (measured 2026-08-25 by
  ...
  names it two lines from the sentence quoted in that row. So a quotation lifted from ch. 2 is
  the likeliest way a banned string reaches this file: **quote a sentence at a time, never a
  paragraph, and run the privacy grep before every commit** (§1). Yeivin's ITM, full
