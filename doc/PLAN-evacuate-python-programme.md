...
4. **book-of-job** — the largest, and the one whose Python is not under a `py/` at all. **Phases 0,
   ...
   **delete** the 40 orphaned UXLC data files. **Phase 6 is what this step now starts on.** All five
   ...
   pick; do not let a later phase pick it silently. **Two root-level files are open questions for
   Ben, and neither is a `.py`**: `book-of-job.code-workspace`, whose five launch configurations
   name deleted scripts, and `requirements.txt`, whose five packages hydrate a `.venv` Phase 7 item
   5 deletes. **At the trio, sweep the repo root for files whose subject is the interpreter rather
   than the code.** Five things these two phases established are worth having before the trio
