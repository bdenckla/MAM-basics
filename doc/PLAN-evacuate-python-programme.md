...
3. **Programme Phase 0** — must precede both remaining plans. **DONE 2026-08-19, all three
   ...
   inside book-of-job).
4. **book-of-job** — the largest, and the one whose Python is not under a `py/` at all. **Phases 0
   and 1 are DONE, 2026-08-19**, gate and all: Ben answered Phase 0's three questions the same day,
   both fork families are reconciled, and Phase 1 landed as `45f8853` there with all 701 artifacts
   byte-identical from a foreign working directory. **Phase 3 is what this step now starts on**,
   and Phase 1 handed it five items, three of them new. The one that is real work: **MAM-basics'
   `py/uxlc_misc/my_uxlc.py` has no `path_to_uxlc` parameter**, its `read()` calling a hardcoded
   `canonical_xml_path(book_id)` where book-of-job's fork takes the path as an argument — so
   Phase 3 must parameterize this repo's copy or point book-of-job's data at `in/UXLC-39/`, and
   book-of-job's nine `py_uxlc_loc/` modules stay a clean deletion only because Phase 1 touched
   none of them. Then: **`py_cam1753_word_image/page.py`'s depth**, correct in book-of-job and
   codex-index-cam1753 alike today and wrong the moment it lands under a `py/`, to be settled in
   both repos at once; **`test_h_dot_below_nfc.py`** folds in as a `_Scope` as holman's did;
   **MAM-basics' venv still lacks `pyspellchecker`**, which `check_spelling_in_html.py` imports at
   module level; and `py/boj_html.py`'s `WriteCtx.path` is now annotated `Path`, which the vendored
   `mb_cmn.file_io` took without change. **One question stands open and is Ben's** — the
   `parents[2]` in the vendored `mb_cmn/provenance.py`, wrong in book-of-job alone of four copies,
   never fired, and deleted outright by Phase 4. Phase 1 did not pick; do not let a later phase
   pick it silently. Five things these two phases established are worth having before the trio
