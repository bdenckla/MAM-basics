...
4. **book-of-job** — the largest, and the one whose Python is not under a `py/` at all. **Its own
   Phase 0 ran 2026-08-19 and stopped at that plan's gate**, so this step is waiting on Ben rather
   than on any work: `mb_cmn/` reconciles, and `py_uxlc_loc/` cannot until three questions are
   answered — whether book-of-job's UXLC snapshot stays at 2.1 or moves to MAM-basics' 2.5, what
   becomes of book-of-job's finer `lci_recs.json` records for Leningrad pages 397A and 406A which
   exist nowhere else, and which of MAM-basics' **two** forks of that code is the target. The
   Status row above carries the evidence. Three things it established are worth having before the
   trio runs, not only here: the oracle rewrites **183** of the 701 artifacts so **518 have no
   generator at all**; each repo's tracked `requirements.txt` has to be installed before its entry
   points can be characterized; and **`git status --porcelain` cannot be the verification here**,
   the byte comparison against `git cat-file blob HEAD:<path>` being what replaces it.
