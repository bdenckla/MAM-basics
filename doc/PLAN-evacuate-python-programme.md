...
|---|---|---|---|---|---|---|---|
| book-of-job | 267 → **268** | 17,060 → 16,859 → **17,064** | 16 → **17** `mb_cmn/` | **DIFFERS** → **identical**, all 17 | `gh-pages` 694, `out` 7 | 2026-08-02 → **2026-08-19** | `static.yml` |
...
4. **book-of-job** — the largest, and the one whose Python is not under a `py/` at all. **Phases 0,
   ...
   run from this repo and on every run since.** **Phase 4 is what this step now starts on**, and it
   stops and asks Ben first, as Phase 3 did. All five items Phase 1 handed Phase 3 are settled, and
   ...
   vendored `mb_cmn.file_io` without change, as predicted. **What Phase 4 now owes beyond its own
   prescription**: book-of-job's 40 UXLC data files are read by nobody once its Python goes, which
   is a question for Ben rather than a mechanical deletion; `pyauthor_util/quirks-BHQ.txt` and
   `check_spelling_in_html.custom-dict.json` are already here, so deleting only `.py` would leave
   `pyauthor_util/` alive as a one-file directory; the NFC scope's floor of 30 assumes 42 files
   survive; and book-of-job's `CLAUDE.md` should say the checks run from MAM-basics now. **One
   question stands open and is Ben's** — the `parents[2]` in the vendored `mb_cmn/provenance.py`,
   wrong in book-of-job alone of four copies, never fired, and deleted outright by Phase 4. Phases
   1 and 3 both declined to pick; do not let a later phase pick it silently. Five things these two phases established are worth having before the trio
