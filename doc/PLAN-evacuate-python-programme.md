...
|---|---|---|---|---|---|---|---|
| book-of-job | 267 | 17,060 → **16,859** | 16 → **17** `mb_cmn/` | **DIFFERS** → **identical**, all 17 | `gh-pages` 694, `out` 7 | 2026-08-02 → **2026-08-19** | `static.yml` |
...
3. **Programme Phase 0** — must precede both remaining plans. **DONE 2026-08-19, all three
   ...
   inside book-of-job).
4. **book-of-job** — the largest, and the one whose Python is not under a `py/` at all. **Its own
   Phase 0 is DONE, 2026-08-19**, gate and all: Ben answered the three questions the same day, and
   both fork families are reconciled with all 701 artifacts byte-identical. **Phase 1 is what this
   step now starts on**, and Phase 0 handed it two items — the `parents[2]` in the vendored
   `mb_cmn/provenance.py`, which lands on `GitRepos` from a root-level `mb_cmn/`, and the data-root
   parameter in `py_uxlc_loc/`, which is the same two-roots problem. The Status row above carries
   the evidence. Three things Phase 0 established are worth having before the trio runs, not only
   here: the oracle rewrites **183** of the 701 artifacts so **518 have no generator at all**; each
   repo's tracked `requirements.txt` has to be installed before its entry points can be
   characterized, and installing book-of-job's made `check_all.py` run there for the first time;
   and **`git status --porcelain` cannot be the verification here**, the byte comparison against
   `git cat-file blob HEAD:<path>` being what replaces it.
