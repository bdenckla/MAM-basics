...
plan is the model and the precedent; this file is the umbrella over the **seven public repos that
still hold Python**, and it owns the three things no single repo's plan can own: the scope, the
order, and the work that must happen **before** any repo moves.
...
| codex-index-aleppo, -leningrad, -cam1753 | [PLAN-evacuate-python-from-codex-index-trio.md](PLAN-evacuate-python-from-codex-index-trio.md) | **not started** |
| diffable-pointed-hebrew | Appendix B below — **a decision, not a plan** | **awaiting Ben** |
| MAM-simple | Appendix A below — **nothing to evacuate** | closed, no work |
...
## Why these seven, and why public-only
...
| diffable-pointed-hebrew | 9 | 561 | 8 `mb_cmn/` | **DIFFERS** | none tracked | 2026-04-27 | none |
| **total** | **548** | **64,398** | | | | | |

For comparison, wlc-utils was **267 files / 59,870 lines** in one repo. This programme is the same
volume of code again, spread over seven repos with seven different shapes — which is why it is
seven plans and not one. MAM-basics currently tracks **693** `.py`; it would end near **1,180**.
...
- **"Flat namespacing, minimal renames — none collide."** That was true of wlc-utils and is
  emphatically not true here. Landing these seven repos flat into `py/` would collide on at least
...
**5. Four of the seven were committed to on 2026-08-02, and two have not moved since 2026-04-27.**
book-of-job, UXLC-utils, holman-ketiv-qere and codex-index-aleppo are live; codex-index-cam1753 and
diffable-pointed-hebrew are dormant. **Files change under you mid-session** — Ben edits in
...
## Appendix B — diffable-pointed-hebrew: a decision for Ben, not a plan

Nine tracked `.py`, of which **eight are a diverged vendored `mb_cmn`** and one is the repo's
entire reason to exist, `diffable-pointed-hebrew.py` at the root. Nineteen tracked files in total,
no `out/`, no `gh-pages/`, no Pages workflow, dormant since 2026-04-27.

Evacuating one script leaves a public repo holding a README and a diverged copy of somebody else's
library. That is worse than either alternative, so the question is which alternative:

- **Move the script to MAM-basics and retire the repo** — it becomes an entry point there,
  the diverged `mb_cmn` is resolved against the originals and deleted, and the repo is archived.
- **Leave it alone** — one script and one dormant repo is not the maintenance tax this programme
  exists to reduce, and archiving a public repo is visible in a way the others' changes are not.

**Either way the eight diverged `mb_cmn` files are a real finding and should be diffed**, because
a divergence nobody has looked at is a bug of unknown size whichever answer wins. That diff is
worth doing before the decision, not after.
