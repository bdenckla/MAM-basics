...
The following state was measured on 2026-09-08 before this plan was committed:
...
`Add Leningrad Aleppo evidence page`, task id `01a08138-c643-7082-a7ee-b60c4c6a4a06`. Its branch
edits `py/author_site/post_stress_meteg.py` and generated post-stress-meteg pages, which overlap the
planned post-stress remediation. Do not start the post-stress wave until that task has completed its
scheduled integration or until a fresh check proves the planned paths do not overlap. Do not merge,
move, or edit that worktree on this plan's authority.

Re-establish the checkout, branch, head, remote head, status, worktree list, and branch divergence
before editing. The plan commit itself advances `main`, and later work may advance both branches, so
the hashes above are evidence, not expected implementation heads. Inspect every intervening commit
touching a finding's paths. A later edit to a path is not proof that the finding was fixed.

The seven post-stress pages regenerated byte-identically at the review anchor, but both their
generator and outputs changed after the anchor. This planning phase did not rerun the survey, the
authored-site generator, a product generator, or the mega pipeline. Generated-artifact synchrony at
the implementation head is therefore a preflight question, not an established baseline.
...
| 3 | Fix after remeasurement | Wave 2. Collapse qamats-variant entries to MAM chanted words for census denominators and counts; use the Sol correction of 368 rows and 370 duplicated chanted words; investigate the nine deḥi-versus-meteg differences and the two unexplained prose entries before accepting new figures. |
...
| 12 | Re-measure, then fix | Wave 2. Reconcile every post-stress record and plan with the current JSON and current page set. The active post-stress branch has already changed some named paths, so no 231/232, 3,181/496, page-count, or fragment statement may be copied without regeneration. |
| 13 | Fix | Wave 2. Make the type-3 definition and classifier agree, finish the type-2 name change, and replace the unsupported BHS claim with claims supported by the actual UXLC/WLC inputs and manuscript crop. |
| 14 | Fix licence coverage | Wave 5. Add precise rows for manuscript crops and post-stress outputs, correct landed/source directory descriptions and MAM statement locations, cover every Taamey D font copy, and replace vague authorship vocabulary. Read the live image provenance before naming a rights holder or photograph source. |
...
| 21 | Fix prose | Waves 2 and 5. Apply the `hebrew-prose` skill and global prose rules to the live post-stress pages, source prose, comments, plans, review disposition, and announced counts. Re-scan current files because the active branch has already changed the named pages. |
...
- [ ] Wait for or explicitly coordinate with the active `post-stress-meteg` task before editing any
      overlapping file. Merge no branch merely to satisfy this checkbox.
...
- [ ] Repair `README.md`, `DATA-LICENSES.md`, and every affected landed README. Number every announced
      count and keep GPL, MIT, corpus-text, font, image, and source/product directory scopes distinct.
