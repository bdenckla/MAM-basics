...
## Phase 5 — Flip the provenance breadcrumbs — DONE 2026-08-01

**Landed as `ce7bf96` here (the source) and `fcbf7b1` in wlc-utils (the regenerated
breadcrumbs).** Every baseline was re-measured first and every one matched: 693 tracked `.py`
and 69 test modules here, 832 passed / 5 skipped, and in wlc-utils 0 tracked `.py`, 193 under
`out/`, 283 under `gh-pages/`, 61 breadcrumb files.

**The verification ran as specified and passed, at exactly the predicted size.** From
`C:\Users\BenDe\GitRepos\MAM-basics` as the working directory: `py\main_0_mega.py`, then the
seven accgram subcommands outside it (run-dual-cant, run-printed-decalogue,
survey-chanted-word-accents, xcheck-poetic, servi-xcheck, test-fixes, grammaticality), then
`main_uxlc_grammar_test.py`, `main_find_uxlc_accent_changes.py` and
`main_edition_transcription.py build --check` (12/12 committed `.txt` bodies re-derived).
wlc-utils' `git diff --stat` was **61 files, 61 insertions, 61 deletions**, and the diff body
was **122 lines, every one a `generated by` line** — 61 removals reading `wlc-utils/py/...` and
61 additions reading `MAM-basics/py/...`, nothing else. Afterwards
`git grep -lI "generated by wlc-utils" -- out gh-pages in` is **0** and the same grep for
`generated by MAM-basics` is **61**. The other 415 tracked artifacts, which carry no breadcrumb,
and `in/accgram/uxlc_accent_changes.json` came back unchanged; `git status --porcelain` showed
nothing but the source edits in MAM-basics and was empty in all five MAM siblings. No stray
`__pycache__` appeared in wlc-utils.

Final measurements here: **830 passed, 5 skipped** (832 less the two tests of the deleted
wrapper); `ruff check py` clean; `black --check py` clean at 690 files (692 less the two
deletions).

The mega alone accounts for 57 of the 61 files. The remaining four come from the entry points
outside it, so **a session that stops after the mega sees a diff that looks nearly right and is
not** — the same structural gap that left `out/accgram/uxlc_grammar_test.txt` stale before
Phase 1 found it.

Five things went differently from what is written underneath:

- **`py/tests/test_wlc_provenance.py` was deleted, not repointed at the code root.** The
  instruction below asks for it to be updated, but its subject module is gone, and
  `test_mb_cmn_provenance.py` already tests the override mechanism from both sides — an explicit
  `repo_name` replacing the top segment, and an omitted one falling back to
  `paths.repo_root().name`. A fresh assertion that the string reads
  `MAM-basics/py/main_accgram.py` would be exactly the example-based unit test `CLAUDE.md` rules
  out, and the guard that actually fires is the 61-file diff above.
- **The `logical_name` override is now dead code in this repo, and was deliberately left in
  place.** After the flip nothing outside `test_mb_cmn_provenance.py` passes `repo_name` to
  `mb_cmn.provenance`. Removing the parameter was not taken because `provenance.py` is vendored
  into MAM-simple, UXLC-utils, al-hatorah and wlc-utils, so an edit there would ripple into
  sibling repos and widen a phase whose whole verification is "the diff is nothing but
  `generated by` lines". One consequence for a later phase: `_display_path`'s docstring still
  offers wlc-utils as the motivating example and says the override lets a repo "still emit
  stable `wlc-utils/...` paths", which no longer describes anything that happens.
- **Regenerating from a MAM-basics git worktree will now write the worktree's directory name
  into the breadcrumbs.** `REPO_NAME` made these 61 artifacts worktree-proof; without it
  `_display_path` uses `repo_root.name`, which under `.claude/worktrees/agent-xxx` is
  `agent-xxx`. This is not a regression so much as the end of an exemption — every other
  artifact MAM-basics generates, into MAM-parsed, MAM-simple, MAM-OSIS and the rest, has always
  behaved this way — but it does mean a worktree run produces 61 spurious diffs. Worth knowing
  before reading one as damage.
- **`printed_decalogue.run`'s deferred provenance import was un-deferred.** Its comment gave
  "to keep this module's import graph light" as the reason, which was already weak — the module
  imports `wlc_paths` at the top anyway — and is void once the import is `mb_cmn.provenance`,
  which pulls in nothing but `pathlib`. The import moved to the top block and the comment kept
  only its live half, that `transcription_parse` imports this module back and would cycle.
- **Two comments asserted a distinction the flip erases.** `classify._write_json` and
  `rtms_output._write_json` each explained not passing `file_io`'s `generator_file=` on the
  ground that `wlc_provenance` "names the repo where `file_io`'s `generator_file=` would not".
  `mb_cmn.provenance` and `file_io`'s `generator_file=` now derive the identical string, so both
  comments were rewritten to the reason that survives: the caller has already injected
  provenance onto the payload off its own `__file__`.

One trap worth recording for any future bulk edit here: **`Path.write_text` translates `\n` to
`\r\n` on Windows**, so a first pass at the 20-file import rewrite silently converted whole
files to CRLF. `git diff` still showed one changed line per file, because git normalizes on
read — the only visible symptom was a "CRLF will be replaced by LF" warning per file. Reverted
and redone with `open(..., newline="")` on both the read and the write.
...
## Phase 6 — Disambiguate issue citations — DONE 2026-08-02

**Landed as `06986c2` here and `3494646` in wlc-utils.** Every baseline was re-measured first.
Three matched exactly — **830 passed, 5 skipped** here, and in wlc-utils **0** tracked `.py`, 193
under `out/`, 283 under `gh-pages/`, **61** files carrying a `generated by MAM-basics` breadcrumb
and **0** carrying `generated by wlc-utils`. Two differed from the figures the Phase 6 task was
handed, both harmlessly: **691** tracked `.py` here, as recorded, but **68** test modules rather
than 69 — Phase 5 deleted `test_wlc_provenance.py` and its own record says so, so the "69" was
simply carried forward one phase too far.

**326 citations across 318 lines in 94 files took the prefix**, resolving 50 distinct numbers.
`ruff check py` is clean, `black --check py` is clean at 690 files, and the suite is unchanged at
**830 passed, 5 skipped**.

**The regeneration ran the full Phase 5 circuit** — `py\main_0_mega.py`, the seven accgram
subcommands outside it (run-dual-cant, run-printed-decalogue, survey-chanted-word-accents,
xcheck-poetic, servi-xcheck, test-fixes, grammaticality), then `main_uxlc_grammar_test.py`,
`main_find_uxlc_accent_changes.py` and `main_edition_transcription.py build --check` (12/12
committed `.txt` bodies re-derived). Afterwards **all five MAM siblings were clean**, and
wlc-utils held exactly the two artifacts named below and nothing else: 474 of the 476 tracked
artifacts, plus `in/accgram/uxlc_accent_changes.json` at 450 records, came back byte-identical.

Five things went differently from what is written underneath:

- **Three of the seven numbers this phase names as "genuine MAM-basics" are not issues at all.**
  `#194`, `#221` and `#246` cite **sections of Yeivin's ITM**, which the moved code writes in the
  same `#NN` shape (`# See Yeivin ITM #194 (page 167)`), as does the whole `#325`–`#391` poetic
  run in `poetic_ply_grammar.py` and `hebrew_accents.py`. Left bare. Only `#187`, `#198`, `#201`
  and `#208` of that list are real MAM-basics issues.
- **`#60` goes the other way, and the phase does not list it.**
  `check_repo_standards.py:194`'s "three issue-#60 vendoring handoffs" is MAM-basics #60, "Get
  vendoring under control" — wlc-utils #60 is about patax-vs-qamats at the printed-tradition
  tirtsax. Left bare. It is the one number where the naive reading ("a number under 88 in moved
  code is wlc-utils'") would have produced exactly the plausible-looking lie this phase warns of.
- **`#75` splits within itself**, four sites two ways: `edition_transcription.py:153` and
  `printed_decalogue_strands.py:85` mean wlc-utils' "make maqaf a token of its own", while
  `mb_cmn/paths.py:3` and `check_repo_standards.py:13` mean MAM-basics' cwd-versus-`__file__`
  convention. A per-number rule could not have got this right.
- **Two of the rewritten citations are read by an artifact, not by a programmer**, so the wlc-utils
  commit is not empty: `maqaf_nonfinal_accents._NAMED_CONFIGURATIONS`' label "no ITM section names
  it; issue #86" (13 lines of `out/accgram/maqaf-nonfinal-accents.json`) and
  `chanted_word_accents`' `open_question` string, which cites `#86` and `#82` (one line of
  `out/accgram/chanted-word-accents.json`). **They were prefixed rather than left alone**, on the
  ground that the sentence a reader would misread is the one in MAM-basics' source; the artifacts
  are downstream of that decision, not a second ledger. The wlc-utils diff is 14 lines and every
  one is an issue reference, so the phase's "the commit's whole diff is issue-reference text"
  holds across both repos. That is a judgment call, not a derivation — the opposite reading, that
  an `out/` file is read in wlc-utils where a bare `#NN` is still correct, is available and was
  weighed.
- **Eleven sites keep a bare `#NN` deliberately**, and are listed in `06986c2`'s message so a later
  sweep does not "fix" them: the Ps 17:14 accent indices `(#7-8)`, `(#9)` and `(#10)` in
  `poetic_ply_grammar.py:763`; three sites whose prose already names the repo beside the number
  (`check_memory_health.py:58`, `check_repo_standards.py:13`, `test_h_dot_below_nfc.py:2`);
  `printed_decalogue_fetch.py:73`'s User-Agent, which already opens `"wlc-utils/"`; the two
  MAM-basics `#75`s; and `wlc_issue_edit.py:26`, whose bare `#69` **is** the worked example of the
  ambiguity, in the docstring of the tool whose required `repo` argument removes it.

Also worth carrying forward: four `wlc-utils#NN` citations already existed in the moved code
before this phase (`dual_cant_detangle.py:74`, `lexical_validation.py:80`,
`transcription_build.py:17`, `transcription_check.py:44`), written that way in wlc-utils itself,
so the convention was not invented here. And the bulk rewrite used `open(..., newline="")` on both
the read and the write, per Phase 5's trap: its own diff was 318 insertions against 318 deletions,
one line for one line, with no file converted to CRLF. (The commit is 348/318 because
`CLAUDE.md` gained the policy section, and because black split one docstring's closing `"""` in
`printed_decalogue_page.py` once the added ten characters pushed that line past 88.)
...
wlc-utils **0** tracked `.py`, **193** under `out/`, **283** under `gh-pages/`, **61** files
carrying a `generated by MAM-basics` breadcrumb and **0** carrying `generated by wlc-utils`.
Final measurements here: **830 passed, 5 skipped** unchanged, `ruff check py` clean,
`black --check py` clean at **691** files (690 plus the new `repo_util/clean_worktrees.py`).

**The regeneration ran the full Phase 5/6 circuit and passed.** `py\main_0_mega.py`, the seven
accgram subcommands outside it (run-dual-cant, run-printed-decalogue,
survey-chanted-word-accents, xcheck-poetic, servi-xcheck, test-fixes, grammaticality), then
`main_uxlc_grammar_test.py`, `main_find_uxlc_accent_changes.py` and
`main_edition_transcription.py build --check` (12/12 committed `.txt` bodies re-derived).
Afterwards `git status --porcelain` was empty in **wlc-utils**, in MAM-basics, and in MAM-parsed,
MAM-OSIS and MAM-for-Sefaria. The two siblings that were not empty were both predicted, and are
the two extra commits above: MAM-simple's re-vendored `mb_cmn/file_io.py` and
`mb_cmn/provenance.py` (comments only — the ripple Phase 5 declined to start), and
MAM-with-doc's `gh-pages/change-log/unpinned-latest.html`, whose window ends at "today" so it
re-dates on the first rebuild of any new day while still reporting 0 changes since `9ce6ee5`.

Nine things went differently from what is written underneath. The first four are corrections to
the items themselves:

- **Item 1 was not cosmetic — `py/main_vendoring.py` had been dead since Phase 4.** With the
  `wlc-utils` entry still in `in/vendoring_policy.json`, `--all` does not produce a stale
  inventory, it raises `ValueError: Configured scan root does not exist:
  ...\wlc-utils\py\mb_cmn`. Measured directly, by restoring the pre-Phase-7 policy file and
  re-running. So the vendoring audit was broken from 2026-08-01 and nothing noticed, because it
  is run by neither `main_test.py` nor `main_0_mega.py`. That is the same structural gap that
  left `out/accgram/uxlc_grammar_test.txt` stale before Phase 1 found it.
- **Item 2's diff does NOT remove only wlc-utils rows, and almost none of the excess is this
  plan's doing.** The inventory had simply not been regenerated since April. Measured by
  regenerating the pre-Phase-7 tree against today's siblings: **23 rows / 174 files**, against
  the committed **26 rows / 189 files**. al-hatorah has picked up `mpplus_schema_guard.py`; a
  dozen copies that read `identical` have diverged (`hebrew_accents.py` in holman-ketiv-qere and
  mgketer, last synced 2026-07-06; three `mb_diff_mpu` files in holman-ketiv-qere; `uni_heb.py`
  and `my_utils.py` in al-hatorah), while MAM-simple's `uni_heb.py` and `styles_mam_parsed.py`
  have converged the other way. Exactly **one** row is Phase 7's: `file_io.py` and
  `provenance.py` split out as `DIFFERS` wherever they are vendored, because of the comment edits
  below — 23 rows without them, **24** with. The wlc-utils row itself is −1 row, −20 files.
- **Item 5's "no code change needed" was wrong, and the miss was consequential.**
  `_has_tracked_py_files()` was consulted only *after* the sweep failed to FIND a black, so it
  never fired for a Python-less repo that still had a venv. wlc-utils is exactly that: Phase 4
  deliberately left its 80 MB venv on disk, and its three orphaned agent worktrees each hold a
  full pre-evacuation checkout — so `black .` there found and reformatted **789 `.py` files
  tracked in no index anywhere**. They were black-clean that day, so nothing changed; the next
  version bump would have rewritten all 789 inside checkouts nobody was going to commit. The gate
  now runs before black is even looked for, and the sweep reports
  `BLACK_ATTEMPTED=False; Skipped: no tracked .py files in this repo`, which is what item 5
  predicted it already did.
- **Item 4's gate keeps the two worktree counts, against the item's wording.** `maintenance_script`,
  `path_utility` and the `script_covers` half of `worktree_hygiene` read `n/a` for a repo with no
  tracked `.py`, on a new per-repo `has_tracked_py`. `linked_worktrees` and `agent_branches` are
  deliberately NOT gated: they stay true, and item 6 exists precisely because such a repo goes on
  accruing worktrees. Gating them would have hidden wlc-utils' `LINKED_WORKTREES=3`.

The rest:

- **The same defect as item 5's, in a second tool, caught only because item 2 writes a tracked
  artifact.** `py/vendoring/provenance.py` finds copy scripts with `rglob`, excluding only
  `__pycache__` and `.venv` — so it found holman-ketiv-qere's two copy scripts twice, the second
  time under `.claude/worktrees/festive-shamir-dad9ec/`, and was about to commit that path into
  `out/vendoring_provenance_out.{txt,json}`. Fixed in `1086805`: `.claude` and `.git` added, and
...
- **`check_repo_standards.py` on an emptied wlc-utils reports no false findings** — the check
  ...
  **12 NFC findings, every one in `in/`** — `in/UXLC-39/Psalms.xml`, `in/UXLC-misc/all_changes.json`
  and the derived `in/accgram/uxlc_accent_changes.json`. Those are external UXLC snapshots, the
  exact noise class the module's own docstring names, and unchanged by the evacuation.
- **Two stale references the plan's eight do not name were fixed too**, both false as written
  rather than merely dated: `mb_cmn/hebrew_accent_strip.py` credited wlc-utils with
  `accgram/hebrew_verse_sanitize.py`, and `mb_cmn/file_io.py` with accgram's per-book corpora.
  Together with `mb_cmn/provenance.py`'s `_display_path` docstring — Phase 5's banked finding, the
  `repo_name` override nothing passes any more — these are the vendored files whose edit ripples,
  and MAM-simple's `137c0a9` is that ripple.
- **`accgram/gen_highlight_picker.py` now cites the commits, not the plan file.** Phase 4
  established that `doc/PLAN-sys-path-insert.md` has never been tracked in any repo while the work
  it names was really done, so the docstring names wlc-utils `3083859` and `36d7693` and says why
  not the file, so the citation is not "restored" later.
- **THE TEST SUITE AND EVERY GENERATOR MUST BE RUN FROM THE MAIN CHECKOUT, NOT A WORKTREE**, and
  this cost a first pass here. `py/main_test.py` from `.claude/worktrees/<name>` gives **12 failed,
  813 passed, 5 skipped, 5 errors** even with `REPOS_ROOT` set, because
  `mb_cmn/read_books_from_mam_parsed_plus.py` keeps a cwd-relative `"../MAM-parsed"` on purpose
  (it is vendored, per CLAUDE.md's named exception), and from a worktree that resolves to
  `.claude/MAM-parsed`. The same run from the repo root is **830 passed, 5 skipped**. Generators
  are worse than merely broken there: they succeed and write the worktree's directory name into
  the breadcrumbs, per Phase 5's third bullet — `doc/vendoring-inventory.md`'s own
  `generated by MAM-basics/py/vendoring/gen_inventory.py` header is one of those. Edit in the
  worktree if you like; run from `C:\Users\BenDe\GitRepos\MAM-basics`.
- **wlc-utils moved under this session**, `3494646` → `72ba4ba`, "Bump the pinned Pages actions off
  the deprecated Node 20". Unrelated to this plan, and a live instance of the last risk below.
