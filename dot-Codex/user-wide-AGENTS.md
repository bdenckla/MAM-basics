...
- **A worktree is the exception, because there the branch is the mechanism, not a
  ...
  worktree was.)
- **Name a new Codex-managed worktree's branch after its directory.** Ben's decision,
  2026-09-10: when starting work in a new Codex-managed worktree, verify the actual checkout
  path. If HEAD is detached, create and switch to a branch at the current HEAD named
  `codex/worktrees/<worktree-id>/<repository-directory-name>` before editing. For example,
  `.../worktrees/a3ff/MAM-basics` gets branch `codex/worktrees/a3ff/MAM-basics`.
  Preserve an existing checked-out branch. If the proposed branch name already exists,
  inspect its commit and worktree association before proceeding; do not overwrite it.
  The app creates the worktree before the agent starts; this instruction governs branch
  creation after startup.
