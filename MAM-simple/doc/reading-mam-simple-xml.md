...
```xml
<!-- This file was generated by MAM-basics/py/main_mam_simple.py. -->
<!-- consumer_notice: {"summary":"...","critical_rules":["..."],"documentation":"https://github.com/bdenckla/MAM-basics/blob/main/MAM-simple/doc/reading-mam-simple.md#consumer-notice"} -->
...
The two comments precede the root. The second comment contains the same notice object as
the JSON format, serialized after the `consumer_notice: ` label; its actual summary and
rule strings are not abbreviated in generated files. The comment does not change the
XML element schema. The root element is always `<book24>`, and its one attribute is
`versification-tradition`. See the canonical [consumer notice](reading-mam-simple.md#consumer-notice).
...
This text is therefore neither NFC nor NFD. A literal search using Unicode-normalized
input can miss equivalent-looking MAM text, and normalization can change bytes without
changing the display. Preserve MAM order when a byte-for-byte round trip or
MAM-compatible output is required. A consumer whose output contract requires another
order can transform deliberately, but should transform both sides before comparison
rather than assume rendered equality means byte equality.
...
Element boundaries are not segmentation boundaries. Select the required Scripture
branches and reassemble their adjacent fragments in document order before identifying
atoms or chanted words. An atom is one written form between spaces or maqafs. A chanted
word is one atom or a complete maqaf compound. Neither a storage fragment nor a
punctuation element becomes a separate atom or chanted word merely because it has its
own tag.
...
The children and `slhw-desc-0` are two representations of the same atom-form. Consume
the children when letter formatting matters or `slhw-desc-0` when an uninterrupted
atom-form is enough; never append both to the running text.
