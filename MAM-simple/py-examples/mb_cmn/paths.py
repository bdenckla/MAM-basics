...
def mam_simple_dir() -> Path:
    ...
    return repo_root() / "MAM-simple" / "json-vtrad-bhs"


def require_mam_simple_dir() -> Path:
    """``mam_simple_dir``, checked before a reader opens a book file."""
    path = mam_simple_dir()
    if not path.is_dir():
        raise FileNotFoundError(f"MAM-simple's landed JSON is absent: {path}")
    return path
...
def mam_simple_vtrad_mam_dir() -> Path:
    ...
    """
    return repo_root() / "MAM-simple" / "json-vtrad-mam"


def require_mam_simple_vtrad_mam_dir() -> Path:
    """``mam_simple_vtrad_mam_dir``, checked before a reader opens a book file."""
    path = mam_simple_vtrad_mam_dir()
    if not path.is_dir():
        raise FileNotFoundError(
            f"MAM-simple's landed MAM-native JSON is absent: {path}"
        )
    return path
