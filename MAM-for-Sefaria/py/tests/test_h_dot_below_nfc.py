...
"""Enforcement test (vendored from MAM-basics issue #187): hand-authored
source must standardize Latin transliteration of Hebrew het ("h with dot
below") on NFC, i.e. the precomposed U+1E25 / U+1E24 forms, never the
decomposed "h"/"H" + COMBINING DOT BELOW (U+0323) sequence. Comments must
not use either Unicode form at all -- plain ASCII "x"/"X" is used instead,
since comments don't flow to output.
...
the deliberate non-Unicode-standard Hebrew mark order used across these
repos. So this test checks only the specific h-with-dot-below sequence,
which has no such ambiguity.

Repo-specific scoping: MAM-for-Sefaria is almost entirely GENERATED output
of MAM-basics/py/mb_sefaria/mam4sef_or_ajf.py (csv/, csv-ajf/,
misc/unicode-names*/ per-book payloads + their _provenance.md sidecars) plus
GitHub Pages published documentation under gh-pages/. Those are out of scope
here -- they self-correct when their generator emits NFC/LF. This test
covers only the handful of hand-authored top-level files (README.md,
LICENSE.md, config/CI files).

The product root is derived from this test file. `git rev-parse --show-toplevel`
finds the enclosing Git repository only so the tracked-file query can select the
product prefix after the product lands inside MAM-basics. The test uses only the
standard library, so it runs under any interpreter from the product directory.
"""
...
_PRODUCT_PREFIX = (
    ""
    if PRODUCT_ROOT == GIT_ROOT
    else PRODUCT_ROOT.relative_to(GIT_ROOT).as_posix() + "/"
)
...
# Generated-output and published-documentation directory prefixes. Everything
# under these is produced by MAM-basics (csv/, csv-ajf/, misc/unicode-names*/)
# or is GitHub Pages output (gh-pages/); not hand-authored, so out of scope.
_EXCLUDE_DIR_PREFIXES = (
    "csv/",
    "csv-ajf/",
    "misc/unicode-names/",
    "misc/unicode-names-ajf/",
    "gh-pages/",
)
...
def _tracked_files_in_scope():
    result = subprocess.run(
        ["git", "ls-files", "--", _PRODUCT_PREFIX or "."],
        cwd=GIT_ROOT,
        capture_output=True,
        encoding="utf-8",
        check=True,
    ...
    for line in result.stdout.splitlines():
        ...
        if not git_rel:
            continue
        if not git_rel.startswith(_PRODUCT_PREFIX):
            raise AssertionError(f"Tracked product path lacks prefix: {git_rel}")
        posix_rel = git_rel.removeprefix(_PRODUCT_PREFIX).replace("\\", "/")
        if _is_excluded(posix_rel):
            continue
        full = PRODUCT_ROOT / posix_rel
        if not full.is_file():
            continue
        if _is_binary(full):
            continue
...
class TestHDotBelowNfc(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # excludes everything). This repo is mostly generated output, so the
        # hand-authored scope is small -- floor kept well below the actual
        # scoped count (~6 files: README.md, LICENSE.md, .gitignore,
        # .gitattributes, CI + scratch config).
